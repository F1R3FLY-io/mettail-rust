...
bag }, let rest = p0_rest.clone(), let x = binder_0.clone(), let p = body_0.clone(), if is_fresh(& x, & rest), let p1 = (Proc :: PNew(mettail_runtime :: Scope :: from_parts_unsafe(x.clone(), Box :: new(Proc :: PPar({ let mut bag = mettail_runtime :: HashBag :: new();
...
relation pamb_proj_c3_b0_p0(Proc, Name, Name, Proc, mettail_runtime :: HashBag < Proc > , Proc);

pamb_proj_c3_b0_p0(parent.clone(), n.clone(), m.clone(), p.clone(), rest.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
...
bag }, let n = elem_f0_val.clone(), let m = elem_f1_inner_f0_elem_0_f0_val.clone(), let p = elem_f1_inner_f0_elem_0_f1_val.clone(), let rest = elem_f1_inner_f0_rest.clone();
...
relation pamb_proj_c3_b1_p0(Proc, Name, Name, Proc, Proc, mettail_runtime :: HashBag < Proc > , Proc);

pamb_proj_c3_b1_p0(parent.clone(), m.clone(), n.clone(), p.clone(), r.clone(), rest.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
    if let Proc :: PAmb(elem_f0, elem_f1) = elem,
...
bag }, for (elem_f1_inner_f0_elem_1, _count_elem_f1_inner_f0_1) in elem_f1_inner_f0.iter(), if & elem_f1_inner_f0_elem_1 != & elem_f1_inner_f0_elem_0, let m = elem_f1_inner_f0_elem_0_f1_inner_f0_elem_0_f0_val.clone(), let n = elem_f1_inner_f0_elem_0_f0_val.clone(), let p = elem_f1_inner_f0_elem_0_f1_inner_f0_elem_0_f1_val.clone(), let r = elem_f1_inner_f0_elem_1.clone(), let rest = elem_f1_inner_f0_elem_0_f1_inner_f0_rest.clone();
...
rw_proc(parent, result) <--
    pamb_proj_c3_b0_p0(parent, cap_n_p0, cap_m_p0, cap_p_p0, cap_rest_p0, elem_0),
...
rw_proc(parent, result) <--
    pamb_proj_c3_b1_p0(parent, cap_m_p0, cap_n_p0, cap_p_p0, cap_r_p0, cap_rest_p0, elem_0),
