...
pub fn parse_ascent_program_text(text: &str) -> syn::Result<AscentProgram> {
   ...
   syn::parse::Parser::parse2(
      |input: syn::parse::ParseStream| match vendored::ascent_syntax::parse_ascent_program(input, dummy_path.clone())? {
         Either::Left(program) => Ok(program),
         Either::Right(_) => Err(syn::Error::new(input.span(), "include_source! is not supported in runtime parsing")),
      },
      tokens,
   )
}
...
pub fn extract_relations(program: &AscentProgram) -> Vec<(String, Vec<String>, bool)> {
   program
      ...
      .map(|rel| {
         ...
         let types: Vec<String> = rel.field_types.iter().map(|ty| quote::quote!(#ty).to_string()).collect();
