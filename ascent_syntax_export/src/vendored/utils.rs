...
use super::syn_utils::path_get_ident_mut;
...
pub fn punctuated_try_unwrap<T, P, E>(
    punc: Punctuated<Result<T, E>, P>,
) -> Result<Punctuated<T, P>, E> {
    let mut res = Punctuated::new();
    for pair in punc.into_pairs() {
        let (t, p) = pair.into_tuple();
        res.push_value(t?);
...
fn check_lazy_set_contains<T: Hash + Eq>(
    hs: &mut HashSet<T>,
...
#[allow(unused)]
pub fn dedup_all_keep_last<T: Eq>(vec: &mut Vec<T>) {
    dedup_all_keep_last_by(vec, |x, y| x == y);
}
