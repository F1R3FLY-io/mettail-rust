...
impl Parse for CallGoalNode {
   fn parse(input: ParseStream) -> Result<Self> {
      ...
      let _with_kw: kw::with = input.parse().map_err(|_| {
         input.error("call goal requires `with schema_name` (e.g. `call(r, x, y) with edge_schema`)")
      })?;
   ...
   RuleNode {
      body_items: rule
         ...
         .map(|bi| match bi {
            ...
            BodyItemNode::Call(cl) => BodyItemNode::Call(call_desugar_pattern_args(cl, &mut gensym)),
            _ => bi,
         })
         .collect(),
