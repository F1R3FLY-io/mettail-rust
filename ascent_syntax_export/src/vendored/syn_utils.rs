...
mod proc_macro {
    ...
    impl From<proc_macro2::TokenStream> for TokenStream {
        fn from(_: proc_macro2::TokenStream) -> Self {
            TokenStream
        }
    }
}
    ...
    match pat {
        ...
        Pat::Or(or_pat) => {
            ...
            let intersection = cases_vars
                .reduce(|case_vars, accu| collect_set(case_vars.intersection(&accu).cloned()));
...
pub fn expr_visit_free_vars_mbm(expr: reft([Expr]), visitor: &mut dyn FnMut(reft([Ident]))) {
    macro_rules! visit {
        ($e: expr) => {
            expr_visit_free_vars_mbm(reft([$e]), visitor)
        };
    }
    ...
    macro_rules! visit_except {
        ($e: expr, $excluded: expr) => {
            expr_visit_free_vars_mbm($e, visitor_except!($excluded))
        };
    }
    match expr {
        Expr::Array(arr) => {
...
pub fn token_stream_replace_ident(
    ts: TokenStream,
    visitor: &mut dyn FnMut(&mut Ident),
) -> TokenStream {
    fn token_tree_replace_ident(
        mut tt: TokenTree,
        visitor: &mut dyn FnMut(&mut Ident),
    ) -> TokenTree {
        match tt {
