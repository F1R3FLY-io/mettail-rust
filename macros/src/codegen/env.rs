...
pub fn generate_environments(theory: &TheoryDef) -> TokenStream {
    ...
    let category_envs: Vec<TokenStream> = categories.iter().map(|export| {
    ...
    let env_struct_name = format_ident!("{}Env", theory_name);
    
    let field_defs: Vec<TokenStream> = categories.iter().map(|export| {
        let cat_name = &export.name;
    ...
    let field_defaults: Vec<TokenStream> = categories.iter().map(|export| {
        let cat_name = &export.name;
        let field_name = format_ident!("{}", cat_name.to_string().to_lowercase());
        let env_name = format_ident!("{}Env", cat_name);
    ...
    let field_clears: Vec<TokenStream> = categories.iter().map(|export| {
        let cat_name = &export.name;
        let field_name = format_ident!("{}", cat_name.to_string().to_lowercase());
    ...
    let field_empty_checks: Vec<TokenStream> = categories.iter().map(|export| {
        let cat_name = &export.name;
        let field_name = format_ident!("{}", cat_name.to_string().to_lowercase());
...
mod tests {
    use super::*;
    use crate::ast::theory::{TheoryDef, Export};
    use syn::parse_quote;
    ...
    fn test_generate_environments() {
        ...
        let output = generate_environments(&theory);
