...
fn generate_at_sigil_wrap_predicate(language: &LanguageDef) -> TokenStream {
        ...
        .map(|lang_type| {
                ...
                .map(|rule| {
                    ...
                    match leading_operand_field_index(rule).filter(|_| bracket_closed) {
                        Some(idx) => {
                            ...
                            quote! {
                                #cat::#label(#(#binds),*) => {
                                    !__lead.__renders_sigil_leading()
                                },
                            }
                        },
            ...
            quote! {
                impl #cat {
                    ...
                    /// rule whose leading operand renders terminal-leading is NOT
                    /// operand-leading on the page, so it takes the bare spelling. See
                    /// [`Self::__renders_sigil_leading`].
                    #[allow(dead_code)]
    ...
    let terminal_leading_impls: Vec<TokenStream> = language
        ...
        .map(|lang_type| {
            let cat = &lang_type.name;
            let arms = terminal_leading_arms_for(cat, language);
            quote! {
                impl #cat {
                    /// Grammar-derived: `true` iff this term's RENDERED surface begins with a
                    /// SIGIL — a NON-ident-shaped grammar literal (`@`, `(`, `{`, `[`) spelled
                    /// by its own rule, or — for an operand-leading rule — by the leading
                    /// operand's, transitively.
                    ///
                    /// An IDENT-shaped opener (`error`, `Nil`, `Map(`) does NOT count: a keyword
                    /// primary is what the sigil's operand parser takes and stops at, so a tail
                    /// after it is lost. Measured: `@error.union(X)` does not parse, while
                    /// `@@Nil!(0,)` and `@(@Nil)!(0,)` do.
                    ///
                    /// Well-founded: every recursive arm descends strictly into a child.
                    /// Generated by `generate_at_sigil_wrap_predicate`.
                    #[allow(dead_code)]
                    pub fn __renders_sigil_leading(&self) -> bool {
                        match self {
                            #(#arms)*
                            _ => false,
                        }
...
fn generate_sigil_operand_wrap_gate(
    ...
    use crate::gen::syntax::synonymy::{nullary_filler_surfaces, sample_surface_for};
    if sigil_operand_cats.is_empty() {
        return quote! {};
    }
    ...
    for rule in &language.terms {
        ...
        // Render the frame either side of the operand slot, filling the other params.
        let render_side = |items: &[SyntaxExpr]| -> Option<String> {
...
/// ★ THE `__renders_sigil_leading` ARMS, and why this predicate has to exist.
///
...
/// exactly the rows whose leading operand does NOT render terminal-leading.
fn terminal_leading_arms_for(cat: &syn::Ident, language: &LanguageDef) -> Vec<TokenStream> {
    language
        .terms
        .iter()
        .filter(|rule| rule.category.to_string() == cat.to_string())
