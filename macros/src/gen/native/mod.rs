...
/// Original richer native-type vocabulary, shared without a second classifier.
pub use mettail_grammar_core::NativeType;

/// Observe a macro native type using the original last-segment helper.
pub trait NativeTypeFromSynType {
    fn from_syn_type(ty: &syn::Type) -> Self;
}

impl NativeTypeFromSynType for NativeType {
    fn from_syn_type(ty: &syn::Type) -> Self {
...
pub fn native_type_element_ident(native_type: &syn::Type) -> Option<Ident> {
    mettail_ast::language::element_ident_from_native_type(native_type)
}
