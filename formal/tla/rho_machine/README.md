...
proofs. It models four independent RhoNet COMM redexes, `A`, `B`, `C`, and
`D`, and a single completion observation `Q`.

The model checks two kinds of facts:

1. Bounded safety with Apalache: traces remain well typed, completion occurs
   only after all four independent inputs, trace observations match scheduler
...
2. Weak-fairness liveness with TLC: if the scheduler is weakly fair for `A`,
   `B`, `C`, `D`, and `Complete`, then `<> completed` holds.
