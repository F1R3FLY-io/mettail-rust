...
const APP_ID_K_BODY: &str = "lambda`(lam x. x, lam a. lam b. a)`";

// ── Beat 0 (from source) — the FLT construction reflects to ⟦App(id, K)⟧ ────────────────────────

/// The `lambda`(…, …)`` FLT, lowered in construction position, reflects BYTE-FOR-BYTE to the
...
/// `@"fltX"!(lambda`(id, K)`) | for( @lambda`(${f}, K)` <- @"fltX" ){ @"OUT"!(f) }`, written in FLT
...
async fn beat1_from_source_typed_hole_binds_the_function_position() {
    ...
    let fired = out_values_from_source(
        "@\"fltX\"!(lambda`(lam x. x, lam a. lam b. a)`) | \
...
/// Against the source pattern `for( @lambda`(${f}, K)` <- … )`, `⟦App(id, K)⟧` matches (arg = ⟦K⟧,
...
/// and its body RE-QUOTES `lambda`(${f}, ${k})`` (a construction-position `PFlt` whose holes are the
...
async fn beat4_from_source_requote_reconstructs_app() {
    ...
    let out = out_values_from_source(
        "@\"fltX\"!(lambda`(lam x. x, lam a. lam b. a)`) | \
         for( @lambda`(${f}, ${k})` <- @\"fltX\" ){ @\"OUT\"!(lambda`(${f}, ${k})`) }",
    )
    .await;
    assert_eq!(
        out,
...
/// `(λx.x) K` — the FLT subject `lambda`(id, K)`` built FROM SOURCE — is seeded to the backend's
...
/// The SOURCE receive pattern `for( @lambda`(${f}, K)` <- … )` requires the unforgeable `GPrivate`
...
/// A `&`-join whose FIRST bind is an FLT typed-hole pattern (`@lambda`(${f}, K)` <- fltX`) and whose
...
async fn join14_from_source_flt_hole_and_moniker_binder_co_bind() {
    ...
    let mut out = out_values_from_source(
        "@\"fltX\"!(lambda`(lam x. x, lam a. lam b. a)`) | @\"other\"!(lambda`lam a. lam b. a`) | \
         for( @lambda`(${f}, lam a. lam b. a)` <- @\"fltX\" & @g <- @\"other\" ){ \
         @\"OUT\"!(f) | @\"OUT\"!(g) }",
    )
    .await;
