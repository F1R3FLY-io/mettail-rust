...
//! surface syntax (`lam`(…, …)``) through the `PFlt` arms (`lower_proc` construction →
//! `reflect_flt_construction`; the receive-pattern path → `reflect_flt_pattern`), with a
//! `FltRegistry` mapping the opener tag `"lam"` → `LambdaLanguage`.
//!
...
/// The `"lam"` → `LambdaLanguage` FLT resolver every from-source lowering installs.
...
const APP_ID_K_BODY: &str = "lam`(lam x. x, lam a. lam b. a)`";

// ── Beat 0 (from source) — the FLT construction reflects to ⟦App(id, K)⟧ ────────────────────────

/// The `lam`(…, …)`` FLT, lowered in construction position, reflects BYTE-FOR-BYTE to the
...
/// Lower a from-source RhoCalc FLT program (installing the `lam` guest) and run it to rest,
...
/// `@"fltX"!(lam`(id, K)`) | for( @lam`(${f}, K)` <- @"fltX" ){ @"OUT"!(f) }`, written in FLT
...
async fn beat1_from_source_typed_hole_binds_the_function_position() {
    ...
    let fired = out_values_from_source(
        "@\"fltX\"!(lam`(lam x. x, lam a. lam b. a)`) | \
         for( @lam`(${f}, lam a. lam b. a)` <- @\"fltX\" ){ @\"OUT\"!(f) }",
    )
    .await;
...
/// Against the source pattern `for( @lam`(${f}, K)` <- … )`, `⟦App(id, K)⟧` matches (arg = ⟦K⟧,
...
/// and its body RE-QUOTES `lam`(${f}, ${k})`` (a construction-position `PFlt` whose holes are the
...
/// `(λx.x) K` — the FLT subject `lam`(id, K)`` built FROM SOURCE — is seeded to the backend's
...
/// The SOURCE receive pattern `for( @lam`(${f}, K)` <- … )` requires the unforgeable `GPrivate`
...
async fn beat3_from_source_pattern_rejects_counterfeit() {
    ...
    let receive_proc =
        Proc::parse("for( @lam`(${f}, lam a. lam b. a)` <- @\"fltX\" ){ @\"OUT\"!(f) }")
