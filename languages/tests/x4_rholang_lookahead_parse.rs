//! X4 — the two lookahead productions **in the real Rholang grammar**.
//!
...
//! the bracket question from Rholang's ~700 other productions — but it is not evidence about
//! Rholang itself, whose `List` type owns `[`/`]` as its `open_parts`/`close_parts` and whose
...
//! long note in `rholang-runtime/src/bin/rholang.rs` on why the latter is not term-preserving).
//!
...
fn read_ok(source: &str) -> String {
    read(source).unwrap_or_else(|err| panic!("Rholang must parse {source:?}: {err}"))
}
