//! Ambiguity goldens for the Rholang **semantic-predicate** surface.
//!
//! Every new production added to `rholang.rs` widens the grammar, and this
...
//! explosion, see `rholang_tests.rs`'s F3 pins). So each semantic-predicate form
...
//! `cargo nextest run -p languages --test rholang_semantic_predicate_ambiguity`.
//!
...
//!
//! ```text
//!   touch languages/src/rholang.rs && cargo build -p languages --all-features 2>&1 \
...
fn ppar_forms_are_unambiguous() {
    ...
    // because Rholang runs `options { reserved_keywords: auto }`, which reserves
