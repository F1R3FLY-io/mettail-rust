...
:: refinementsmoke; #[test] fn
parity_refinementsmoke_posint_cross_cat_int_0000()
{
    let input = "0"; let legacy = refinementsmoke :: PosInt :: parse(input);
    let wpds = refinementsmoke :: PosInt :: parse_via_wpds(input); match
    (legacy, wpds)
    {
        (Ok(a), Ok(b)) =>
        {
