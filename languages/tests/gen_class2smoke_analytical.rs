...
#[test] fn ambiguity_exposure_class2smoke_00()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 2usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 2usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_01()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 3usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 3usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_02()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 4usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 4usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_03()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 5usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 5usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_04()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 6usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 6usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_05()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 7usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 7usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_06()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 8usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 8usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_07()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 9usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 9usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_08()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 10usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
    (drained [0].message.contains(& 10usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_class2smoke_09()
{
    clear_diagnostics(); emit_d11("Class2Smoke", "TestCat", 11usize); let
    drained = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq!
    (drained [0].language, "Class2Smoke"); assert!
...
binder_shadowing_class2smoke_00()
{
    ...
    (paths [0].len(), 2usize);
} #[test] fn binder_shadowing_class2smoke_01()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 3usize);
} #[test] fn binder_shadowing_class2smoke_02()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 4usize);
} #[test] fn binder_shadowing_class2smoke_03()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 5usize);
} #[test] fn binder_shadowing_class2smoke_04()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 6usize);
} #[test] fn binder_shadowing_class2smoke_05()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 7usize);
} #[test] fn binder_shadowing_class2smoke_06()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 8usize);
} #[test] fn binder_shadowing_class2smoke_07()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 9usize);
} #[test] fn binder_shadowing_class2smoke_08()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 10usize);
} #[test] fn binder_shadowing_class2smoke_09()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
...
cross_cat_ambiguity_class2smoke_00()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_01()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_02()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_03()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_04()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_05()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_06()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_07()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_08()
{
...
} #[test] fn cross_cat_ambiguity_class2smoke_09()
{
...
pratt_bp_boundary_class2smoke_00()
{
...
} #[test] fn pratt_bp_boundary_class2smoke_01()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_02()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_03()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_04()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_05()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_06()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_07()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_08()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_class2smoke_09()
{
...
recovery_corruption_class2smoke_00()
{
    ...
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_01()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_02()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_03()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_04()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_05()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_06()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_07()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_08()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_class2smoke_09()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
