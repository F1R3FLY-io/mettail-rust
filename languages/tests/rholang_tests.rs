//! Systematic test suite for the Rholang language.
//!
...
//   • non-COMM  — `RholangLanguage::dovetail_normal_term` (native folds, casts, `Exec`/drop,
...
mod oracle {
    use super::Proc;
    use mettail_languages::rholang::{RholangLanguage, RholangTerm, RholangTermInner};
    ...
    fn to_term(p: &Proc) -> RholangTerm {
        RholangTerm(RholangTermInner::Proc(p.clone()))
    }

    fn proc_from_inner(inner: &RholangTermInner) -> Option<Proc> {
        match inner {
            RholangTermInner::Proc(p) => Some(p.clone()),
            _ => None,
        }
    }
    ...
    fn collect_proc_alts(inner: &RholangTermInner, out: &mut Vec<Proc>) {
        match inner {
            RholangTermInner::Proc(p) => out.push(p.clone()),
            RholangTermInner::Ambiguous(alts) => {
                for a in alts {
    ...
    pub fn run_fixpoint(input: &str) -> (AscentResults, u64) {
        ...
        if let Some(t) = box0.as_any().downcast_ref::<RholangTerm>() {
        ...
        if !root_is_pnew {
            if let Ok(nf) =
                RholangLanguage::dovetail_normal_term(box0.as_ref(), DOVETAIL_ITERS, DOVETAIL_NODES)
            {
...
/// display. That is the right thing whenever the value has a Rholang source spelling that parses
...
/// Normalize a Rholang normal-form *display* to the bare surface the pre-merge
...
/// leaked into semantics: `*(@(1)) + 2` answered `error`, collection methods received mismatched
/// keys/indices, and a
...
/// `languages/src/rholang.rs`: the integer literal domains now PARTITION (`Int` = every ≤`i64`
...
mod numeral_carrier_is_context_independent {
    ...
    /// auto-injected `IntToBigInt` promotion). Reducer-backed method tests pin the observable
    /// lookup consequence; this layer pins the carrier structure itself.
    #[test]
...
mod comm {
    ...
    fn comm_with_body_using_channel() {
        ...
        // DECLARED canonical member `NQuoteShort` (`rholang.rs`), so the surface is the Rholang
    ...
    //                       (`languages/src/rholang/receive.rs`), reached through
    ...
    /// (`reduce.rs::combine_eq`), which answers `true` for two `GBool`s; Rholang
...
mod congruence {
    ...
    fn add_cong() {
        ...
        // arbitrary precision, `*(@(1)) + int(2, 64)` in fixed width — because Rholang's `+` is
...
mod native_ops {
    ...
    // Method-call evaluation lives in f1r3node's reducer and is exercised through the
    // reducer-backed rholang-runtime suites.  This language-layer suite retains only
    // collection syntax and non-method rewrite behavior; duplicating method semantics here
    // would recreate the registry E3 removed.
    mod collection_surface {
        use super::*;

        #[test]
        fn pathmap_literal_participates_in_receive_patterns() {
    ...
    //
    // The six golden-hex tests that lived here pinned `languages/src/rholang/wire.rs`, a
    ...
    //     (`sint64` zigzag: `…100210041006` = 1, 2, 3), but a plain Rholang integer literal is
    //     arbitrary-precision, so the term Rholang actually means carries `GBigInt` elements.
    //
    // The replacements live in `rholang-runtime/tests/rho_rholang_conformance.rs`
...
mod parsing {
    ...
    fn send_empty_payload_quoted_bind_emits_empty_proc() {
        ...
        // divergence is recorded in `rholang-runtime/tests/rho_rholang_conformance.rs`.
...
#[test]
fn rholang_cast_int_float_floor() {
...
#[test]
fn rholang_cast_int_invalid_width_error() {
...
#[test]
fn rholang_cast_int_nonfinite_float_is_error() {
...
#[test]
fn rholang_cast_uint_float_clamp() {
...
#[test]
fn rholang_cast_uint_modular_u32_literal() {
...
#[test]
fn rholang_cast_float_overflow_to_inf() {
...
#[test]
fn rholang_cast_float_from_rational_string() {
...
#[test]
fn rholang_cast_float_from_bigint_n_string() {
    assert_reduces_to(r#"float("1000n", 64)"#, "1000.0");
}

#[test]
fn rholang_cast_float_from_fixed_p_string() {
    assert_reduces_to(r#"float("1000.1p1", 64)"#, "1000.1");
}

#[test]
fn rholang_casts_from_numeric_strings() {
...
#[test]
fn rholang_str_from_rational_literal() {
    assert_reduces_to(r#"str(23r)"#, r#""23""#);
}

#[test]
fn rholang_bigint_unary_from_float() {
...
#[test]
fn rholang_cast_int_congruence_through_add() {
    assert_reduces_to("int(1 + 2, 8)", "3");
}

#[test]
fn rholang_cast_uint_signed_int_twos_complement() {
...
#[test]
fn rholang_cast_under_send_reduces_via_comm() {
...
// gen_rholang_prop::proc_display_parse_roundtrip minimized to
