// AUTO-GENERATED by language! macro for AcBagDemo (unit) — do not edit
...
#[test]
fn acbagdemo_language_instantiates() {
    let lang = AcBagDemoLanguage;
    assert_eq!(lang.name(), "AcBagDemo");
}

#[test]
fn acbagdemo_metadata_populated() {
    let lang = AcBagDemoLanguage;
...
#[test]
fn unit_acbagdemo_proc_mark() {
    mettail_runtime::clear_var_cache();
    let term = Proc::Mark(std::sync::Arc::new(Proc::A));
    let displayed = format!("{}", term);
    assert!(!displayed.is_empty(), "Display should produce non-empty output for Mark");
    if let Ok(parsed) = Proc::parse(&displayed) {
        let re_displayed = format!("{}", parsed);
        assert_eq!(
            displayed, re_displayed,
            "Roundtrip failed for Mark: {} != {}",
            displayed, re_displayed
        );
    }
}
...
#[test]
fn unit_acbagdemo_auto_proc_pvar() {
    mettail_runtime::clear_var_cache();
...
#[test]
fn unit_acbagdemo_auto_bag_baglit() {
