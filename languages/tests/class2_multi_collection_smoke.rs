...
// Task #11 (extended 2026-07-26): `Class2Multi` is a FIXTURE grammar, not a production
// language, so its definition lives in `languages/tests/definitions/class2multi.rs` rather
...
// only invoker of the opt-in `class2multi_generated_tests!` wrapper, which materializes the
// macro-generated unit / prop / analytical sections that used to be written to
// `languages/tests/gen_class2multi_*.rs`. Other consumers `#[path]`-include the same definition
// WITHOUT invoking the wrapper, so the generated tests exist exactly once across the suite.
#[path = "definitions/class2multi.rs"]
mod class2multi;

class2multi::class2multi_generated_tests!(crate::class2multi);

use class2multi::Proc;

#[test]
fn pred1_both_empty() {
    let result = Proc::parse_via_wpda("pair ( ) ( )").expect("empty-empty Pair parses");
    match &result {
        Proc::Pair(xs, ys) => {
...
fn pred2_first_singleton() {
    let result = Proc::parse_via_wpda("pair ( 0 ) ( )").expect("singleton-empty Pair parses");
    match &result {
        Proc::Pair(xs, ys) => {
            assert_eq!(xs.len(), 1);
...
fn pred3_second_singleton() {
    let result = Proc::parse_via_wpda("pair ( ) ( 0 )").expect("empty-singleton Pair parses");
    match &result {
        Proc::Pair(xs, ys) => {
            assert_eq!(xs.len(), 0);
            assert_eq!(ys.len(), 1);
...
fn pred4_both_nonempty() {
    let result =
        Proc::parse_via_wpda("pair ( 0 | 0 ) ( 0 )").expect("nonempty-nonempty Pair parses");
    match &result {
        Proc::Pair(xs, ys) => {
            assert_eq!(xs.len(), 2);
            for x in xs.iter() {
