//! Rholang `new` vs official Rholang `new` — surface, scope, and ambiguity pins.
//!
//! Rholang IS Rholang. On 2026-07-24 the `new` production dropped its
//! Rholang-only GROUPING PARENTHESES, realigning the declaration list with both
//! normative sources:
//!
//! | source | production |
//! |---|---|
...
//! measured one. Official Rholang's body is any `_proc`; making Rholang's body a
...
fn retired_grouping_paren_form_is_rejected() {
    // The pre-2026-07-24 Rholang-only shape is GONE — a clean break, no alias.
...
fn the_body_is_a_required_block_a_known_divergence() {
    // Official Rholang accepts any `_proc` as the body. Rholang requires the
    ...
    for src in ["new x in Nil", "new x in x!(0)", "new x in 1 + 2", "new x in a or b"] {
        ...
        assert!(
            Proc::parse(src).is_err(),
            "`{src}`: Rholang's `new` body is currently a REQUIRED block (§17.10-B1)",
        );
    }
}
...
fn empty_decl_list_is_accepted_a_known_divergence() {
    ...
    // — at least ONE declaration. Rholang's PNew-style binder loop is lowered
    ...
    assert_eq!(
        ...
        0,
        "Rholang accepts a zero-binder `new`; official Rholang does not",
    );
}
...
// ══════════════════════════════════════════════════════════════════════════
//
// Dropping the grouping parens made a multi-binder `new` the first Rholang
...
//               in the template. Found by `gen_rholang_prop`, which is seeded
...
fn a_new_in_a_plain_comma_fenced_slot_is_grouped() {
    ...
    //
    //   gen_rholang_prop::name_display_parse_roundtrip
...
fn a_native_collection_literal_recovers_without_grouping() {
    ...
    match &parse(&canonical) {
        Proc::CastList(l) => match &**l {
            mettail_languages::rholang::List::ListLit(items) => assert_eq!(
                items.len(),
                2,
