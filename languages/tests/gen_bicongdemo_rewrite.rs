// AUTO-GENERATED by language! macro for BiCongDemo (rewrite) — do not edit
...
#[test]
fn rewrite_bicongdemo_flip() {
    let _lang = BiCongDemoLanguage;
    let meta = _lang.metadata();
    let rewrites = meta.rewrites();
    // Verify rewrite Flip (index 0) exists in metadata
    assert!(
        rewrites.len() > 0,
    ...
    assert_eq!(rw.name, Some("Flip"), "Rewrite rule name mismatch");
    assert!(!rw.lhs.is_empty(), "Rewrite Flip LHS should be non-empty");
    assert!(!rw.rhs.is_empty(), "Rewrite Flip RHS should be non-empty");
}

// Concrete execution test: run "swap ( 0 , 0 )" through SimulationRunner,
// assert it reaches NormalForm (exercises rewrite rule Flip).
#[test]
fn rewrite_bicongdemo_flip_exec() {
    ...
    if let Ok(Ok(trace)) = result {
        assert!(
            ...
            "Rewrite rule Flip: input 'swap ( 0 , 0 )' did not reach NF: {:?}",
            trace.outcome,
        );
    }
...
// Rewrite test for NodeCong skipped: congruence rule (needs triggering context)
#[test]
fn rewrite_bicongdemo_nodecong() {
    let _lang = BiCongDemoLanguage;
