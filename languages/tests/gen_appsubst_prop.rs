// AUTO-GENERATED by language! macro for AppSubst (prop) — do not edit
...
impl AnyTerm {
    ...
    fn unwrap_t(self) -> T {
        match self {
            AnyTerm::WrapT(v) => v,
            _ => panic!("AnyTerm::unwrap_t: wrong variant"),
        }
    }
}
...
#[allow(dead_code)]
enum BuildTask {
    /// Build a T term at the given depth, storing result in the given slot.
    BuildT { depth: u32, slot: usize },
}
...
/// Build a `T` term from an instruction tape.
///
...
fn build_t_from_tape(reader: &mut TapeReader<'_>, depth: u32) -> T {
    if depth == 0 {
        let result = {
            let _ = reader.next_byte(); // consume tape byte for replay determinism
            AnyTerm::WrapT(T::TVar(mettail_runtime::OrdVar(mettail_runtime::Var::Free(
                mettail_runtime::get_or_create_var("a"),
            ))))
        };
        return result.unwrap_t();
    }
    ...
    match choice {
        ...
        1 => {
            ...
            let body = build_t_from_tape(reader, child_depth);
            let scope = mettail_runtime::Scope::new(binder, std::sync::Arc::new(body));
            T::Abs(scope)
        },
        _ => {
            let f0 = std::sync::Arc::new(build_t_from_tape(reader, child_depth));
            let f1 = std::sync::Arc::new(build_t_from_tape(reader, child_depth));
            T::Ap(f0, f1)
        },
    }
}

/// Generate an arbitrary `T` term with bounded depth.
///
/// Uses a flat `Vec<u8>` tape interpreted by `build_t_from_tape`.
/// Proptest shrinking produces shorter tapes = simpler terms.
#[allow(dead_code)]
fn arb_t(max_depth: u32) -> BoxedStrategy<T> {
    ...
    proptest::collection::vec(any::<u8>(), 1..max_tape)
        .prop_map(move |tape| {
            ...
            build_t_from_tape(&mut reader, max_depth)
        })
        .boxed()
}
...
proptest! {
    ...
    #[test]
    fn t_debug_does_not_panic(term in arb_t(4)) {
        let _ = format!("{:?}", term);
    }

    #[test]
    fn t_display_does_not_panic(term in arb_t(4)) {
        let _ = format!("{}", term);
    }

    #[test]
    fn t_clone_eq(term in arb_t(4)) {
        let cloned = term.clone();
        prop_assert_eq!(term, cloned);
    }

    #[test]
    fn t_display_parse_roundtrip(term in arb_t(3)) {
        ...
        if __grind_diag { eprintln!("GRIND-T-GEN ast={:?}", term); }
        let displayed = format!("{}", term);
        if __grind_diag { eprintln!("GRIND-T-DISP disp={:?}", displayed); }
        ...
        let parsed = T::parse(&displayed)
            .unwrap_or_else(|e| panic!(
                "arb_t produced unparseable surface term {:?}: {:?}",
                displayed, e));
        if __grind_diag { eprintln!("GRIND-T-PARSE-OK elapsed={:?} disp={:?}", __grind_t0.elapsed(), displayed); }
        let canonical = format!("{}", parsed);
        if canonical.len() > 500 { return Ok(()); }
        let reparsed = T::parse(&canonical).unwrap_or_else(|e| panic!(
    ...
    #[test]
    fn t_strong_roundtrip_via_display(term in arb_t(1)) {
    ...
    #[test]
    fn t_parse_determinism(term in arb_t(2)) {
        mettail_runtime::clear_var_cache();
        let __grind_diag = std::env::var("GRIND_DIAG").is_ok();
        let displayed = format!("{}", term);
        if __grind_diag { eprintln!("GRIND-T-DET-DISP ast={:?} disp={:?}", term, displayed); }
        // Skip terms whose display is too long (parser may overflow)
        if displayed.len() > 500 {
            return Ok(());
        }
        mettail_runtime::clear_var_cache();
        let p1 = T::parse(&displayed);
        mettail_runtime::clear_var_cache();
        let p2 = T::parse(&displayed);
        match (p1, p2) {
            (Ok(t1), Ok(t2)) => {
                let d1 = format!("{}", t1);
                let d2 = format!("{}", t2);
...
#[test]
fn sim_appsubst_normal_form_reachability() {
    ...
    let test_inputs: Vec<&str> = vec!["abs  0 . 0", "ap ( 0 , 0 )"];
    ...
    if tested > 0 {
        ...
        assert!(
            pass_rate >= 0.80,
            "AppSubst normal form reachability: only {:.1}% reached NF ({} / {})",
            pass_rate * 100.0,
            reached_nf,
            tested,
        );
...
#[test]
fn sim_appsubst_roundtrip_under_rewrite() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        let trace2 = match result2 {
            Ok(Ok(t)) => t,
            _ => panic!(
                "AppSubst roundtrip re-run panicked or errored for input {:?} \
                 (first NF: {:?})",
                input, nf1,
            ),
        };
        let nf2 = match &trace2.outcome {
            TraceOutcome::NormalForm { term, .. } => term.clone(),
            other => panic!(
                "AppSubst roundtrip re-run did not reach NF for input {:?} \
                 (first NF: {:?}): {:?}",
                input, nf1, other,
            ),
        };
        ...
        if nf1 != nf2 {
            panic!(
                "AppSubst roundtrip under rewrite FAILED for input {:?}:\n\
                 First NF:  {:?}\nSecond NF: {:?}",
                input, nf1, nf2,
            );
        }
...
#[test]
fn sim_appsubst_morphology_bounded() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let Some(ref morph) = trace.morphology {
                assert!(
                    ...
                    "AppSubst morphology: max nodes {} exceeds bound 50000 for input {:?}",
                    morph.max_nodes,
                    input,
                );
                assert!(
                    morph.max_depth <= 100,
                    "AppSubst morphology: max depth {} exceeds bound 100 for input {:?}",
                    morph.max_depth,
                    input,
                );
            }
...
#[test]
fn sim_appsubst_eval_determinism() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        assert_eq!(
            nf1, nf2,
            "AppSubst eval determinism: different normal forms for input {:?}: {:?} vs {:?}",
            input, nf1, nf2,
        );
    }
}
...
proptest! {
    ...
    #[test]
    fn sim_appsubst_proptest_campaign(term in arb_t(3u32)) {
        use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
        use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let TraceOutcome::InvariantViolation { ref invariant, ref message, .. } = trace.outcome {
                prop_assert!(
                    false,
                    "AppSubst invariant '{}' violated on '{}': {}",
