...
:: basemath; #[test] fn parity_basemath_num_infix_plus_0000()
{
    let input = "0 + 0"; let legacy = basemath :: Num :: parse(input); let
    wpds = basemath :: Num :: parse_via_wpds(input); match (legacy, wpds)
    {
        (Ok(a), Ok(b)) =>
        {
            assert_eq!
...
} #[test] fn parity_basemath_num_infix_minus_0001()
{
    let input = "0 - 0"; let legacy = basemath :: Num :: parse(input); let
    wpds = basemath :: Num :: parse_via_wpds(input); match (legacy, wpds)
    {
