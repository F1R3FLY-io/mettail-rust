// AUTO-GENERATED by language! macro for AcBagDemo (prop) — do not edit
...
fn build_proc_from_tape(reader: &mut TapeReader<'_>, depth: u32) -> Proc {
    if depth == 0 {
        ...
        let result = match choice {
            0 => AnyTerm::WrapProc(Proc::A),
            1 => AnyTerm::WrapProc(Proc::B),
            2 => AnyTerm::WrapProc(Proc::C),
            _ => {
    ...
    match choice {
        0 => AnyTerm::WrapProc(Proc::A).unwrap_proc(),
        1 => AnyTerm::WrapProc(Proc::B).unwrap_proc(),
        2 => AnyTerm::WrapProc(Proc::C).unwrap_proc(),
        3 => {
            let _ = reader.next_byte(); // consume tape byte for replay determinism
            AnyTerm::WrapProc(Proc::PVar(mettail_runtime::OrdVar(mettail_runtime::Var::Free(
                mettail_runtime::get_or_create_var("a"),
...
#[test]
fn sim_acbagdemo_normal_form_reachability() {
    ...
    let test_inputs: Vec<&str> = vec!["A", "B", "C", "mark ( 0 )"];
    ...
    if tested > 0 {
        ...
        assert!(
            pass_rate >= 0.80,
            "AcBagDemo normal form reachability: only {:.1}% reached NF ({} / {})",
            pass_rate * 100.0,
            reached_nf,
            tested,
        );
...
#[test]
fn sim_acbagdemo_roundtrip_under_rewrite() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        let trace2 = match result2 {
            Ok(Ok(t)) => t,
            _ => panic!(
                "AcBagDemo roundtrip re-run panicked or errored for input {:?} \
                 (first NF: {:?})",
                input, nf1,
            ),
        };
        let nf2 = match &trace2.outcome {
            TraceOutcome::NormalForm { term, .. } => term.clone(),
            other => panic!(
                "AcBagDemo roundtrip re-run did not reach NF for input {:?} \
                 (first NF: {:?}): {:?}",
                input, nf1, other,
            ),
        };
        ...
        if nf1 != nf2 {
            panic!(
                "AcBagDemo roundtrip under rewrite FAILED for input {:?}:\n\
                 First NF:  {:?}\nSecond NF: {:?}",
                input, nf1, nf2,
            );
        }
...
#[test]
fn sim_acbagdemo_morphology_bounded() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let Some(ref morph) = trace.morphology {
                assert!(
                    ...
                    "AcBagDemo morphology: max nodes {} exceeds bound 50000 for input {:?}",
                    morph.max_nodes,
                    input,
                );
                assert!(
                    morph.max_depth <= 100,
                    "AcBagDemo morphology: max depth {} exceeds bound 100 for input {:?}",
                    morph.max_depth,
                    input,
                );
            }
...
#[test]
fn sim_acbagdemo_eval_determinism() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        assert_eq!(
            nf1, nf2,
            "AcBagDemo eval determinism: different normal forms for input {:?}: {:?} vs {:?}",
            input, nf1, nf2,
        );
    }
}
...
proptest! {
    ...
    #[test]
    fn sim_acbagdemo_proptest_campaign(term in arb_proc(3u32)) {
        use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
        use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let TraceOutcome::InvariantViolation { ref invariant, ref message, .. } = trace.outcome {
                prop_assert!(
                    false,
                    "AcBagDemo invariant '{}' violated on '{}': {}",
