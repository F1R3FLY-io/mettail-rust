...
:: extmath; #[test] fn parity_extmath_num_infix_plus_0000()
{
    let input = "0 + 0"; let legacy = extmath :: Num :: parse(input); let wpds
    = extmath :: Num :: parse_via_wpds(input); match (legacy, wpds)
    {
        (Ok(a), Ok(b)) =>
        {
            assert_eq!
...
} #[test] fn parity_extmath_num_infix_minus_0001()
{
    let input = "0 - 0"; let legacy = extmath :: Num :: parse(input); let wpds
    = extmath :: Num :: parse_via_wpds(input); match (legacy, wpds)
    {
