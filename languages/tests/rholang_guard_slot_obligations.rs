//! **Rholang's `where` slot induces a semantic-predicate obligation.**
//!
//! # What this pins, and why it needed a new mechanism
//!
//! *"If it is in a `where` clause, it is a semantic predicate."* Before this change,
//! `collect_guard_obligations` induced **zero** obligations from Rholang's `where` surface: it
//! emitted one only for a [`TermParam::GuardBody`] — a `?name:Guard` slot — and Rholang's guard
...
//!
//! | Rholang `where` today | as a `BehavioralPred` |
//! |---|---|
...
//! `mettail_languages::rholang::guard_substrate` encodes into the Dovetail/SFT substrate.
//!
...
fn rholang_def() -> mettail_ast::language::LanguageDef {
...
fn both_where_surfaces_induce_a_behavioral_predicate_obligation() {
    let obligations = collect_guard_obligations(&rholang_def());
    ...
    for expected_id in ["term:ForRowWhere:guard:cond", "term:ForRowSingleWhere:guard:cond"] {
        ...
        assert!(
            ...
            "expected obligation `{expected_id}` (BehavioralPredicate); Rholang's `where` slot \
...
fn a_declared_slot_and_a_typed_slot_induce_the_same_shape() {
    let rholang = collect_guard_obligations(&rholang_def());
    let declared = rholang
        .iter()
...
fn no_undeclared_rule_induces_a_term_guard_obligation() {
    ...
    let declared: Vec<String> = def
        ...
        .unwrap_or_default();
    assert_eq!(declared.len(), 2, "Rholang declares exactly two guard slots");
