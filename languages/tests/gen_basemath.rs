// AUTO-GENERATED by language! macro for BaseMath — do not edit
...
#[test]
fn prec_basemath_sub_add_tighter_than_1___2___3() {
    mettail_runtime::clear_var_cache();
...
#[test]
fn prec_basemath_paren_override_add_sub__1___2____3() {
    mettail_runtime::clear_var_cache();
...
#[test]
fn sim_basemath_normal_form_reachability() {
    ...
    if tested > 0 {
        ...
        assert!(
            pass_rate >= 0.80,
            "BaseMath normal form reachability: only {:.1}% reached NF ({} / {})",
            pass_rate * 100.0,
            reached_nf,
            tested,
        );
...
#[test]
fn sim_basemath_roundtrip_under_rewrite() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        let trace2 = match result2 {
            Ok(Ok(t)) => t,
            _ => panic!(
                "BaseMath roundtrip re-run panicked or errored for input {:?} \
                 (first NF: {:?})",
                input, nf1,
            ),
        };
        let nf2 = match &trace2.outcome {
            TraceOutcome::NormalForm { term, .. } => term.clone(),
            other => panic!(
                "BaseMath roundtrip re-run did not reach NF for input {:?} \
                 (first NF: {:?}): {:?}",
                input, nf1, other,
            ),
        };
        ...
        if nf1 != nf2 {
            panic!(
                "BaseMath roundtrip under rewrite FAILED for input {:?}:\n\
                 First NF:  {:?}\nSecond NF: {:?}",
                input, nf1, nf2,
            );
        }
...
#[test]
fn sim_basemath_morphology_bounded() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let Some(ref morph) = trace.morphology {
                assert!(
                    ...
                    "BaseMath morphology: max nodes {} exceeds bound 50000 for input {:?}",
                    morph.max_nodes,
                    input,
                );
                assert!(
                    morph.max_depth <= 100,
                    "BaseMath morphology: max depth {} exceeds bound 100 for input {:?}",
                    morph.max_depth,
                    input,
                );
            }
...
#[test]
fn sim_basemath_eval_determinism() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        assert_eq!(
            nf1, nf2,
            "BaseMath eval determinism: different normal forms for input {:?}: {:?} vs {:?}",
            input, nf1, nf2,
        );
    }
}
...
proptest! {
    ...
    #[test]
    fn sim_basemath_proptest_campaign(term in arb_num(3u32)) {
        use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
        use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let TraceOutcome::InvariantViolation { ref invariant, ref message, .. } = trace.outcome {
                prop_assert!(
                    false,
                    "BaseMath invariant '{}' violated on '{}': {}",
