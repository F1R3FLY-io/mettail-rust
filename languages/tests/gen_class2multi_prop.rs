// AUTO-GENERATED by language! macro for Class2Multi (prop) — do not edit
...
let coll_0: Vec<_> = (0..num_elems_0).map(|_| {
build_proc_from_tape(reader, child_depth)
}).collect();
            let num_elems_1 = (reader.next_byte() % 4) as usize;
let coll_1: Vec<_> = (0..num_elems_1).map(|_| {
build_proc_from_tape(reader, child_depth)
}).collect();
            Proc::Pair(coll_0, coll_1)
        },
    }
}
