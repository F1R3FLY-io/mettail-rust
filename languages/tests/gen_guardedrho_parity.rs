...
:: guardedrho; #[test] fn parity_guardedrho_proc_term_kw_0000()
{
    let input = "Nil"; let legacy = guardedrho :: Proc :: parse(input); let
    wpds = guardedrho :: Proc :: parse_via_wpds(input); match (legacy, wpds)
    {
        (Ok(a), Ok(b)) =>
        {
            assert_eq!
...
} #[test] fn parity_guardedrho_proc_cross_cat_int_0001()
{
    let input = "0"; let legacy = guardedrho :: Proc :: parse(input); let wpds
    = guardedrho :: Proc :: parse_via_wpds(input); match (legacy, wpds)
    {
        (Ok(a), Ok(b)) =>
        {
            assert_eq!
...
} #[test] fn parity_guardedrho_name_unary___0002()
{
    let input = "@ Nil"; let legacy = guardedrho :: Name :: parse(input); let
    wpds = guardedrho :: Name :: parse_via_wpds(input); match (legacy, wpds)
    {
        (Ok(a), Ok(b)) =>
        {
            assert_eq!
