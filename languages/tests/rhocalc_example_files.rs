...
//! `Err("…Ascent oracle…not installed…")` for every RhoCalc term. The live
//! reducer is the generated whole-box Dovetail normalizer
//! `<Lang>::dovetail_normal_term` — the SAME non-COMM reducer that
//! `numeric_bigint_cast_regressions.rs` and `rhocalc_tests.rs`'s Path-B
...
/// `numeric_bigint_cast_regressions.rs` and `rhocalc_tests::oracle`).
...
fn rhocalc_examples_env_file_loads_and_runs() {
    ...
    // oracle exactly.
    check_env_file(&path, &rhocalc::RhoCalcLanguage, "rhocalc", |term| {
        ...
        // runtime error and reached a valid non-COMM normal form. `rhocalc.txt`
        // is largely such interaction-driven definitions, so — exactly as
        // `rhocalc_tests.rs`'s Path-B `mod oracle` treats a `dovetail_normal_term`
        ...
        match rhocalc::RhoCalcLanguage::dovetail_normal_term(term, DOVETAIL_ITERS, DOVETAIL_NODES) {
            Ok(_) => Ok(()),
