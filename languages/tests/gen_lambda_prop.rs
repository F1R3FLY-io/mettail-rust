...
proptest! {
    ...
    fn term_display_parse_roundtrip(term in arb_term(3)) {
        ...
        if __grind_diag { eprintln!("GRIND-Term-GEN ast={:?}", term); }
        let displayed = format!("{}", term);
        if __grind_diag { eprintln!("GRIND-Term-DISP disp={:?}", displayed); }
        ...
        let parsed = Term::parse(&displayed)
            .unwrap_or_else(|e| panic!(
                ...
                displayed, e));
        if __grind_diag { eprintln!("GRIND-Term-PARSE-OK elapsed={:?} disp={:?}", __grind_t0.elapsed(), displayed); }
    ...
    fn term_parse_determinism(term in arb_term(2)) {
        ...
        let __grind_diag = std::env::var("GRIND_DIAG").is_ok();
        let displayed = format!("{}", term);
        if __grind_diag { eprintln!("GRIND-Term-DET-DISP ast={:?} disp={:?}", term, displayed); }
        // Skip terms whose display is too long (parser may overflow)
        if displayed.len() > 500 {
            return Ok(());
        }
