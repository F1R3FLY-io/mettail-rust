//! **Promoted counterexamples for `class2optsmoke`** — every seed its proptest corpus records,
//! as a named regression test.
//!
//! # Where these came from, and why they are here
//!
//! `languages/tests/gen_class2optsmoke_prop.proptest-regressions` holds 1 seed(s) for inputs
...
//!     target/generated/class2optsmoke/rust_ctor.rs \
//!     languages/tests/gen_class2optsmoke_prop.proptest-regressions
//! ```
//!
...
// Task #11: `class2optsmoke`'s grammar is a test FIXTURE, not a production language — its
// definition lives in `languages/tests/definitions/class2optsmoke.rs` rather than in the `languages`
// library, so it is `#[path]`-included here. This binary is a CONSUMER: it deliberately does
// NOT invoke the `class2optsmoke_generated_tests!` wrapper, because the definition's designated host is its own smoke-test binary and is the sole
// invoker, so the generated suite stays single-instanced across the workspace.
#[path = "definitions/class2optsmoke.rs"]
mod class2optsmoke;
use crate::class2optsmoke::*;

/// Corpus entry 0 — seed `cc 2538b26e7753db1d337d8f8fd8826cd9bf15983a7c8d1e84e642bb3517f4823d`.
///
/// Recorded counterexample, verbatim from the corpus:
/// ```text
/// term = ChooseMaybe(PZero, None)
/// ```
#[test]
fn corpus_0_proc() {
    mettail_runtime::clear_var_cache();
    // 1 — the term CONSTRUCTS.
    let term: Proc = Proc::ChooseMaybe(std::sync::Arc::new(Proc::PZero), None);
    ...
    let recorded = "ChooseMaybe(PZero, None)";
    assert_eq!(
