...
#[test] fn ambiguity_exposure_ambient_00()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 2usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 2usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_01()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 3usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 3usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_02()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 4usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 4usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_03()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 5usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 5usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_04()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 6usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 6usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_05()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 7usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 7usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_06()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 8usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 8usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_07()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 9usize); let drained =
    take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 9usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_08()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 10usize); let drained
    = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
    assert! (drained [0].message.contains(& 10usize.to_string())); assert_eq!
    (pending_count(), 0);
} #[test] fn ambiguity_exposure_ambient_09()
{
    clear_diagnostics(); emit_d11("Ambient", "TestCat", 11usize); let drained
    = take_diagnostics(); assert_eq! (drained.len(), 1); assert_eq!
    (drained [0].code, "D11"); assert_eq! (drained [0].language, "Ambient");
...
binder_shadowing_ambient_00()
{
    ...
    (paths [0].len(), 2usize);
} #[test] fn binder_shadowing_ambient_01()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 3usize);
} #[test] fn binder_shadowing_ambient_02()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 4usize);
} #[test] fn binder_shadowing_ambient_03()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 5usize);
} #[test] fn binder_shadowing_ambient_04()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 6usize);
} #[test] fn binder_shadowing_ambient_05()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 7usize);
} #[test] fn binder_shadowing_ambient_06()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 8usize);
} #[test] fn binder_shadowing_ambient_07()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 9usize);
} #[test] fn binder_shadowing_ambient_08()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
    ...
    (paths.len(), 1, "linear binder chain has one path"); assert_eq!
    (paths [0].len(), 10usize);
} #[test] fn binder_shadowing_ambient_09()
{
    let mut g : WpdaGss < LexicographicWeight > = WpdaGss :: new(); let root =
    g.get_or_create_node(WpdaGssNode
    { pos : 0, symbol : StackSymbolV2 :: category_entry(0), }); let mut cur =
...
cross_cat_ambiguity_ambient_00()
{
...
} #[test] fn cross_cat_ambiguity_ambient_01()
{
...
} #[test] fn cross_cat_ambiguity_ambient_02()
{
...
} #[test] fn cross_cat_ambiguity_ambient_03()
{
...
} #[test] fn cross_cat_ambiguity_ambient_04()
{
...
} #[test] fn cross_cat_ambiguity_ambient_05()
{
...
} #[test] fn cross_cat_ambiguity_ambient_06()
{
...
} #[test] fn cross_cat_ambiguity_ambient_07()
{
...
} #[test] fn cross_cat_ambiguity_ambient_08()
{
...
} #[test] fn cross_cat_ambiguity_ambient_09()
{
...
} #[test] fn pratt_bp_boundary_ambient_01()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_02()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_03()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_04()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_05()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_06()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_07()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_08()
{
    ...
    (a.kind, SymbolKind :: InfixContinuation); assert_eq!
    (b.kind, SymbolKind :: InfixContinuation);
} #[test] fn pratt_bp_boundary_ambient_09()
{
...
recovery_corruption_ambient_00()
{
    ...
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_01()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_02()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_03()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_04()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_05()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_06()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_07()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_08()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
    ...
    w.process_event(WpdaEvent :: Inspect, & empty_tokens); assert!
    (! w.state().is_terminal());
} #[test] fn recovery_corruption_ambient_09()
{
    let mut w : WpdaWalker < LexicographicWeight, _ > = WpdaWalker ::
    new(IdleEngine, 0); let empty_tokens = SliceTokenSource :: new(& []); let
    _ =
