// AUTO-GENERATED by language! macro for BiCongDemo (prop) — do not edit
...
fn build_proc_from_tape(reader: &mut TapeReader<'_>, depth: u32) -> Proc {
    ...
    match choice {
        ...
        3 => AnyTerm::WrapProc(Proc::D).unwrap_proc(),
        4 => {
            let _ = reader.next_byte(); // consume tape byte for replay determinism
            AnyTerm::WrapProc(Proc::PVar(mettail_runtime::OrdVar(mettail_runtime::Var::Free(
                mettail_runtime::get_or_create_var("a"),
...
#[test]
fn sim_bicongdemo_normal_form_reachability() {
    ...
    let test_inputs: Vec<&str> =
        vec!["A", "B", "C", "D", "pair ( 0 , 0 )", "swap ( 0 , 0 )", "node ( 0 , 0 )"];
    ...
    if tested > 0 {
        ...
        assert!(
            pass_rate >= 0.80,
            "BiCongDemo normal form reachability: only {:.1}% reached NF ({} / {})",
            pass_rate * 100.0,
            reached_nf,
            tested,
        );
...
#[test]
fn sim_bicongdemo_roundtrip_under_rewrite() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        let trace2 = match result2 {
            Ok(Ok(t)) => t,
            _ => panic!(
                "BiCongDemo roundtrip re-run panicked or errored for input {:?} \
                 (first NF: {:?})",
                input, nf1,
            ),
        };
        let nf2 = match &trace2.outcome {
            TraceOutcome::NormalForm { term, .. } => term.clone(),
            other => panic!(
                "BiCongDemo roundtrip re-run did not reach NF for input {:?} \
                 (first NF: {:?}): {:?}",
                input, nf1, other,
            ),
        };
        ...
        if nf1 != nf2 {
            panic!(
                "BiCongDemo roundtrip under rewrite FAILED for input {:?}:\n\
                 First NF:  {:?}\nSecond NF: {:?}",
                input, nf1, nf2,
            );
        }
...
#[test]
fn sim_bicongdemo_morphology_bounded() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let Some(ref morph) = trace.morphology {
                assert!(
                    ...
                    "BiCongDemo morphology: max nodes {} exceeds bound 50000 for input {:?}",
                    morph.max_nodes,
                    input,
                );
                assert!(
                    morph.max_depth <= 100,
                    "BiCongDemo morphology: max depth {} exceeds bound 100 for input {:?}",
                    morph.max_depth,
                    input,
                );
            }
...
#[test]
fn sim_bicongdemo_eval_determinism() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        assert_eq!(
            nf1, nf2,
            "BiCongDemo eval determinism: different normal forms for input {:?}: {:?} vs {:?}",
            input, nf1, nf2,
        );
    }
}
...
proptest! {
    ...
    #[test]
    fn sim_bicongdemo_proptest_campaign(term in arb_proc(3u32)) {
        use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
        use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let TraceOutcome::InvariantViolation { ref invariant, ref message, .. } = trace.outcome {
                prop_assert!(
                    false,
                    "BiCongDemo invariant '{}' violated on '{}': {}",
