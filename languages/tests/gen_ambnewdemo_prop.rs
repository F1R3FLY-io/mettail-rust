// AUTO-GENERATED by language! macro for AmbNewDemo (prop) — do not edit
...
fn build_proc_from_tape(reader: &mut TapeReader<'_>, depth: u32) -> Proc {
    if depth == 0 {
        ...
        let result = match choice {
            ...
            1 => AnyTerm::WrapProc(Proc::PA),
            2 => AnyTerm::WrapProc(Proc::PB),
            _ => {
    ...
    match choice {
        ...
        1 => AnyTerm::WrapProc(Proc::PA).unwrap_proc(),
        2 => AnyTerm::WrapProc(Proc::PB).unwrap_proc(),
        3 => {
            let _ = reader.next_byte(); // consume tape byte for replay determinism
            AnyTerm::WrapProc(Proc::PVar(mettail_runtime::OrdVar(mettail_runtime::Var::Free(
                mettail_runtime::get_or_create_var("a"),
...
fn build_name_from_tape(reader: &mut TapeReader<'_>, depth: u32) -> Name {
    if depth == 0 {
        ...
        let result = match choice {
            0 => AnyTerm::WrapName(Name::Na),
            1 => AnyTerm::WrapName(Name::Nb),
            _ => {
                let _ = reader.next_byte(); // consume tape byte for replay determinism
...
#[test]
fn sim_ambnewdemo_normal_form_reachability() {
    ...
    let test_inputs: Vec<&str> =
        vec!["0", "A", "B", "na", "nb", "open ( 0 , 0 )", "0 [ 0 ]", "new ( 0 , 0 )"];
    ...
    if tested > 0 {
        ...
        assert!(
            pass_rate >= 0.80,
            "AmbNewDemo normal form reachability: only {:.1}% reached NF ({} / {})",
            pass_rate * 100.0,
            reached_nf,
            tested,
        );
...
#[test]
fn sim_ambnewdemo_roundtrip_under_rewrite() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        let trace2 = match result2 {
            Ok(Ok(t)) => t,
            _ => panic!(
                "AmbNewDemo roundtrip re-run panicked or errored for input {:?} \
                 (first NF: {:?})",
                input, nf1,
            ),
        };
        let nf2 = match &trace2.outcome {
            TraceOutcome::NormalForm { term, .. } => term.clone(),
            other => panic!(
                "AmbNewDemo roundtrip re-run did not reach NF for input {:?} \
                 (first NF: {:?}): {:?}",
                input, nf1, other,
            ),
        };
        ...
        if nf1 != nf2 {
            panic!(
                "AmbNewDemo roundtrip under rewrite FAILED for input {:?}:\n\
                 First NF:  {:?}\nSecond NF: {:?}",
                input, nf1, nf2,
            );
        }
...
#[test]
fn sim_ambnewdemo_morphology_bounded() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let Some(ref morph) = trace.morphology {
                assert!(
                    ...
                    "AmbNewDemo morphology: max nodes {} exceeds bound 50000 for input {:?}",
                    morph.max_nodes,
                    input,
                );
                assert!(
                    morph.max_depth <= 100,
                    "AmbNewDemo morphology: max depth {} exceeds bound 100 for input {:?}",
                    morph.max_depth,
                    input,
                );
            }
...
#[test]
fn sim_ambnewdemo_eval_determinism() {
    use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
    use mettail_simulation::trace::TraceOutcome;
    ...
    for input in &test_inputs {
        ...
        assert_eq!(
            nf1, nf2,
            "AmbNewDemo eval determinism: different normal forms for input {:?}: {:?} vs {:?}",
            input, nf1, nf2,
        );
    }
}
...
proptest! {
    ...
    #[test]
    fn sim_ambnewdemo_proptest_campaign(term in arb_proc(3u32)) {
        use mettail_simulation::runner::{SimulationConfig, SimulationRunner};
        use mettail_simulation::trace::TraceOutcome;
        ...
        if let Ok(Ok(trace)) = result {
            if let TraceOutcome::InvariantViolation { ref invariant, ref message, .. } = trace.outcome {
                prop_assert!(
                    false,
                    "AmbNewDemo invariant '{}' violated on '{}': {}",
