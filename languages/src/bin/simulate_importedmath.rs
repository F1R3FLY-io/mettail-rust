// AUTO-GENERATED simulation binary for ImportedMath — do not edit
// Regenerated on each compilation of the language definition.
// Run with: cargo run --bin simulate_importedmath -- [OPTIONS]

use clap::Parser;
use mettail_languages::importedmath::strategies::arb_num;
use mettail_languages::importedmath::ImportedMathLanguage;
...
#[derive(Parser)]
#[command(name = "simulate_importedmath")]
#[command(about = "Simulation runner for the ImportedMath language")]
struct Args {
...
fn main() {
    ...
    let regression_path = PathBuf::from("simulate_importedmath.regressions");
    ...
    let strategy = arb_num(3).prop_map(|term| format!("{}", term));

    if args.verbose {
        eprintln!("Running simulation for ImportedMath:");
    ...
    println!("=== ImportedMath Simulation Results ===");
    println!("{}", results);
