// AUTO-GENERATED simulation binary for AppSubst — do not edit
...
#[derive(Parser)]
#[command(name = "simulate_appsubst")]
#[command(about = "Simulation runner for the AppSubst language")]
struct Args {
...
fn main() {
    ...
    let regression_path = PathBuf::from("simulate_appsubst.regressions");
    ...
    let strategy = arb_t(3).prop_map(|term| format!("{}", term));

    if args.verbose {
        eprintln!("Running simulation for AppSubst:");
    ...
    println!("=== AppSubst Simulation Results ===");
    println!("{}", results);
