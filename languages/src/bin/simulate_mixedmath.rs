// AUTO-GENERATED simulation binary for MixedMath — do not edit
// Regenerated on each compilation of the language definition.
// Run with: cargo run --bin simulate_mixedmath -- [OPTIONS]
...
use mettail_languages::mixedmath::MixedMathLanguage;
use mettail_languages::mixedmath::strategies::arb_bool;
use mettail_runtime::Language;
use proptest::strategy::Strategy;
use std::path::PathBuf;

#[derive(Parser)]
#[command(name = "simulate_mixedmath")]
#[command(about = "Simulation runner for the MixedMath language")]
struct Args {
...
fn main() {
    ...
    let regression_path = PathBuf::from("simulate_mixedmath.regressions");
    ...
    let strategy = arb_bool(3).prop_map(|term| format!("{}", term));

    if args.verbose {
        eprintln!("Running simulation for MixedMath:");
    ...
    println!("=== MixedMath Simulation Results ===");
    println!("{}", results);
