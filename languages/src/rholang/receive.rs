...
pub(crate) fn canonicalize_arity_payload(payload: &Proc) -> Proc {
    match payload {
        ...
        Proc::PZero => crate::rholang::runtime::mk_proc_list(vec![]),
        _ => crate::rholang::runtime::mk_proc_list(vec![payload.clone()]),
    }
}
...
pub(crate) fn canonicalize_arity_pattern(pattern: &Proc) -> Proc {
    match pattern {
        ...
        _ => crate::rholang::runtime::mk_proc_list(vec![pattern.clone()]),
    }
}
    ...
    match bind {
        ...
        | InputBind::InputBindQuery(lhs, _, _) => {
            if matches!(lhs.as_ref(), Name::NVar(_)) {
                ...
                Some(crate::rholang::runtime::mk_proc_list(vec![name_pattern_to_proc(
                    lhs.as_ref(),
                )]))
            }
        },
...
pub(crate) fn name_pattern_to_proc(name_pat: &Name) -> Proc {
    ...
    match current {
        ...
        // ★ A MISSING ARM, not a fabrication (2026-07-26). `NParen` (`rholang.rs`'s
...
/// `Proc::GuardThen` (`rholang.rs`'s `GuardThen` rule).
///
...
/// [`crate::rholang::runtime::unary_fallback`], the twin of the `binary_fallback` those three
/// operators use.
///
...
/// # Convention followed
///
/// [`crate::rholang::runtime::unary_fallback`] / `binary_fallback`
/// (`languages/src/rholang/runtime.rs`), whose doc states the rule: *"both operands ground ⇒ the
...
/// rule in `rholang.rs` states the other half: *"A failed operator must never invent a value
/// (cf. `817ae380`) — hence `Proc::Err`, never a fabricated `BoolLit`."* This function was the
/// last boolean-consuming fold in Rholang that did not obey either half.
///
...
pub fn guard_then(cond: &Proc, body: &Proc) -> Proc {
    match cond {
        ...
        _ => crate::rholang::runtime::unary_fallback(cond, || {
...
fn eval_cmp_order(lhs: &Proc, rhs: &Proc) -> Option<Ordering> {
    match (lhs, rhs) {
        ...
        // `GBool`s; Rholang conforms DOWN to Rholang" — was closed in the FOLD lane
        // (`rholang.rs`'s `Eq` arm) and NOT here, in the GUARD lane. There are two `Eq`
...
/// fold in Rholang uses. See that function's disposition table and
...
pub(crate) fn eval_where_comm_single(
    ...
    // `languages/tests/rholang_guard_substrate_wire.rs`.
    match crate::rholang::guard_substrate::eval_guard_disposition_via_substrate(&sub_cond) {
...
pub fn comm_pforjoin_subst(
    ...
    match crate::rholang::guard_substrate::eval_guard_disposition_via_substrate(&acc_cond) {
...
fn collect_input_bindings(
    ...
    // makes the join path agree with the row path (`languages/tests/rholang_tests.rs::comm::
...
fn try_comm_on_pfor_user(
    ...
    match &rows[0] {
        ...
        // now-deleted grammar rules (rholang.rs). A `<=` head now parses as the
...
fn finish_single_comm(
    ...
    let new_center = if recv_ctx.empty_bind {
        ...
        if let Some(c) = recv_ctx.where_cond {
            ...
            match crate::rholang::guard_substrate::eval_guard_disposition_via_substrate(c) {
...
fn output_parts(p: &Proc) -> Option<(Name, Proc)> {
    match p {
        ...
        Proc::POutputEmpty(n) | Proc::PPersistOutputEmpty(n) => {
            Some((n.as_ref().clone(), crate::rholang::runtime::mk_proc_list(vec![])))
        },
        Proc::POutput2Plus(n, a, bs) | Proc::PPersistOutput2Plus(n, a, bs) => {
            ...
            Some((n.as_ref().clone(), crate::rholang::runtime::mk_proc_list(items)))
        },
        _ => None,
    }
}
...
mod guard_then_tests {
    ...
    /// e-graph saturation that invokes it (`RholangLanguage::dovetail_normal_term`, whose
    ...
    fn the_fold_lane_does_not_report_nil_for_a_ground_non_boolean_condition() {
        ...
        let lang = crate::rholang::RholangLanguage;
        let term = lang.parse_term("__guard_then(0, \"BODY\")").expect("parse");
        let nf =
            crate::rholang::RholangLanguage::dovetail_normal_term(term.as_ref(), 64, 1_000_000)
...
mod comm_tests {
    use super::*;
    use crate::rholang::runtime::merge_pp_parallel;
