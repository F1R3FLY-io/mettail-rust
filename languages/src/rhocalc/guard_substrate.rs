...
//!
//! A RhoCalc `where` clause holds an ordinary `Proc`. This module maps that `Proc` into
...
//! semantics that belong to the reducer's own matcher. `crate::rhocalc::formula` warns in as many
...
use crate::rhocalc::receive::GuardDisposition;
        ...
        match atom.kind {
            ...
            GuardAtomKind::Spatial => match fragment {
                Proc::Matches(target, formula) => {
                    from_verdict(crate::rhocalc::formula::host_matches_verdict(target, formula))
                },
                _ => Sat3::DontKnow,
            },
            ...
            GuardAtomKind::StructuralEquality => match fragment {
                Proc::Eq(a, b) => {
                    from_verdict(crate::rhocalc::runtime::compare_collection_equality(a, b))
                },
                Proc::Ne(a, b) => from_verdict(
                    crate::rhocalc::runtime::compare_collection_equality(a, b).map(|v| !v),
                ),
                _ => Sat3::DontKnow,
            },
