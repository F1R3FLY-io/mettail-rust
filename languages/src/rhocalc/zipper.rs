...
//
// A rhocalc `Pathmap` key is a LIST of segments, and `flatten_segments`
...
//
// and, decisively, RhoCalc surface syntax cannot ADDRESS a byte: `descendTo`
// takes a `Proc` and always encodes a whole segment. So the byte-granular
// moves cannot be composed into an enumeration from RhoCalc source at all, no
...
/// Decode an encoded trie key back into the rhocalc `Proc` LIST of its path
...
/// report, because rhocalc cannot construct a root-valued `Pathmap` at all;
...
mod tests {
    ...
    fn walk_db() -> PathMapLit<Proc, Proc> {
        let mut lit = PathMapLit::new();
        lit.insert(key(&[1, 2, 3]), int(100));
        lit.insert(key(&[1, 2, 4]), int(200));
        lit.insert(key(&[2, 1]), int(300));
        lit
    }

    /// Drive the walk to exhaustion, collecting `(path, leaf)` per step.
    fn walk_all(lit: &PathMapLit<Proc, Proc>) -> Vec<(String, String)> {
    ...
    /// The root focus names no entry, so `getPath` errors there. rhocalc cannot
    ...
    //
    // ⚠ REQUIRED TRANSLATION FOR C1 — the seam that routes rhocalc collection
