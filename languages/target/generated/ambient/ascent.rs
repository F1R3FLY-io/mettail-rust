::ascent::ascent_source! {
    ambient_source : relation proc(Proc); #[ds(crate ::eqrel)] relation eq_proc(Proc,
    Proc); #[ds(crate ::dual_indexed)] relation rw_proc(Proc, Proc); relation name(Name);
    #[ds(crate ::eqrel)] relation eq_name(Name, Name); #[ds(crate ::dual_indexed)]
    relation rw_name(Name, Name); relation step_term(Proc); #[ds(crate ::dual_indexed)]
    relation ppar_contains(Proc, Proc); proc(sub.clone()) <- - proc(t), for sub in {
    std::thread_local! { static POOL_PROC_PROC : std::cell::Cell < Vec < Proc >> = const
    { std::cell::Cell::new(Vec::new()) }; } let mut buf = POOL_PROC_PROC.with(| p | p
    .take()); buf.clear(); match t { Proc::PIn(_, f1) => { buf.push(f1.as_ref().clone());
    }, Proc::POut(_, f1) => { buf.push(f1.as_ref().clone()); }, Proc::POpen(_, f1) => {
    buf.push(f1.as_ref().clone()); }, Proc::PAmb(_, f1) => { buf.push(f1.as_ref()
    .clone()); }, Proc::PNew(scope) => { buf.push(scope.inner().unsafe_body.as_ref()
    .clone()); }, Proc::ApplyProc(lam, arg) => { buf.push(lam.as_ref().clone()); buf
    .push(arg.as_ref().clone()); }, Proc::MApplyProc(lam, args) => { buf.push(lam
    .as_ref().clone()); buf.extend(args.iter().cloned()); }, Proc::LamProc(scope) => {
    buf.push(scope.inner().unsafe_body.as_ref().clone()); }, Proc::MLamProc(scope) => {
    buf.push(scope.inner().unsafe_body.as_ref().clone()); }, Proc::ApplyName(lam, _) => {
    buf.push(lam.as_ref().clone()); }, Proc::MApplyName(lam, _) => { buf.push(lam
    .as_ref().clone()); }, Proc::LamName(scope) => { buf.push(scope.inner().unsafe_body
    .as_ref().clone()); }, Proc::MLamName(scope) => { buf.push(scope.inner().unsafe_body
    .as_ref().clone()); }, _ => {}, } let iter_buf = std::mem::take(& mut buf);
    POOL_PROC_PROC.with(| p | p.set(buf)); iter_buf } .into_iter(); name(sub.clone()) <-
    - proc(t), for sub in { std::thread_local! { static POOL_PROC_NAME : std::cell::Cell
    < Vec < Name >> = const { std::cell::Cell::new(Vec::new()) }; } let mut buf =
    POOL_PROC_NAME.with(| p | p.take()); buf.clear(); match t { Proc::PIn(f0, _) => { buf
    .push(f0.as_ref().clone()); }, Proc::POut(f0, _) => { buf.push(f0.as_ref().clone());
    }, Proc::POpen(f0, _) => { buf.push(f0.as_ref().clone()); }, Proc::PAmb(f0, _) => {
    buf.push(f0.as_ref().clone()); }, Proc::ApplyName(_, arg) => { buf.push(arg.as_ref()
    .clone()); }, Proc::MApplyName(_, args) => { buf.extend(args.iter().cloned()); }, _
    => {}, } let iter_buf = std::mem::take(& mut buf); POOL_PROC_NAME.with(| p | p
    .set(buf)); iter_buf } .into_iter(); name(sub.clone()) <- - name(t), for sub in {
    std::thread_local! { static POOL_NAME_NAME : std::cell::Cell < Vec < Name >> = const
    { std::cell::Cell::new(Vec::new()) }; } let mut buf = POOL_NAME_NAME.with(| p | p
    .take()); buf.clear(); match t { Name::ApplyProc(lam, _) => { buf.push(lam.as_ref()
    .clone()); }, Name::MApplyProc(lam, _) => { buf.push(lam.as_ref().clone()); },
    Name::LamProc(scope) => { buf.push(scope.inner().unsafe_body.as_ref().clone()); },
    Name::MLamProc(scope) => { buf.push(scope.inner().unsafe_body.as_ref().clone()); },
    Name::ApplyName(lam, arg) => { buf.push(lam.as_ref().clone()); buf.push(arg.as_ref()
    .clone()); }, Name::MApplyName(lam, args) => { buf.push(lam.as_ref().clone()); buf
    .extend(args.iter().cloned()); }, Name::LamName(scope) => { buf.push(scope.inner()
    .unsafe_body.as_ref().clone()); }, Name::MLamName(scope) => { buf.push(scope.inner()
    .unsafe_body.as_ref().clone()); }, _ => {}, } let iter_buf = std::mem::take(& mut
    buf); POOL_NAME_NAME.with(| p | p.set(buf)); iter_buf } .into_iter(); proc(c1.clone()
    .normalize()) <- - proc(c0), rw_proc(c0, c1), if { use std::hash:: { Hash, Hasher };
    let mut __bcg05_h = std::hash::DefaultHasher::new(); c1.hash(& mut __bcg05_h); let
    __bcg05_hash = __bcg05_h.finish(); thread_local! { static __BCG05_EXPAND :
