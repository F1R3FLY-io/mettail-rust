...
/// The original shared native-kind enum; its syn-free methods live in core.
pub use mettail_grammar_core::NativeKind;

/// AST-local extension for the original shallow native-type classification.
///
/// The core enum cannot have an AST-dependent inherent method. Callers retain
/// `NativeKind::from_syn_type` syntax by importing this trait alongside the
/// re-exported enum. Only Type::Path's last segment is observed; qualification,
/// generic arguments, and QSelf are unchanged unobserved source payloads.
///
/// ```
/// use mettail_ast::language::{NativeKind, NativeKindFromSynType};
/// let native: syn::Type = syn::parse_quote!(module::i32);
/// assert_eq!(NativeKind::from_syn_type(&native), NativeKind::Int32);
/// ```
pub trait NativeKindFromSynType {
    fn from_syn_type(ty: &Type) -> Self;
}

impl NativeKindFromSynType for NativeKind {
    fn from_syn_type(ty: &Type) -> Self {
        let seg = match ty {
        ...
        Self::from_last_path_segment(seg.as_str())
    }
}
