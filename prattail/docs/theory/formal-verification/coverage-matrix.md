...
| Recognizer realization laziness | `RuntimeModel.v` (demand-bounded realize; arena-order) | `facade.rs::__mettail_wpda_collect_prefix`; `wpda_walker.rs` realizers | **GAP**: realizer truncates in **arena order, not weight order** → `realized_prefix_is_eager_firstn` would be vacuous until the Phase 2 cube-pruning fix |
