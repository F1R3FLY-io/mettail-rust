...
| 7 | 2026-05-26 | Restore per-iteration `emit_fire_action` in `IterativeChainAbsorb` arm | `6da30a5` | 4102/0 + tramp 15/0/2 + parity 16/0 | **WIN** −15.06 % (t=−20.90) | **WIN** −15.03 % (t=−33.26) | **WIN** −12.41 % (t=−25.59) | **WIN** −8.77 % (t=−20.53) | OOM 24 GB at 6:26 wall (~3.7 GB/min; falsifier 2× E6 = 3.2 GB/min TRIGGERED, marginal 16 % over) | **KEEP per user Welch criterion** (Welch ACCEPT all 4 chain sizes p<0.0001; chain_10000 still OOMs but no better/worse outcome from revert; user override of Plan-agent falsifier explicit) |
| 10 S0 | 2026-05-26 | ConfigKey pairwise correlation matrix instrumentation (`PairCounts` newtype + 10×10 lower-triangular tally + scope-mark histograms) | `67d4a3e` | 4102/0 | n/a (feature-off zero-cost) | n/a | n/a | n/a | n/a | **ACCEPT** — gate FIRES: `(node, edge) = 100.0 %` co-divergence with both sole-diffs 0.002 %; `(node, sppf_top) = 81.7 %`, `(edge, sppf_top) = 81.7 %`. Mechanical recommendation: drop one of {node, edge}. |
| 10 S1 | 2026-05-26 | Drop `incoming_edge` axis from `ConfigKey` (`wpda_walker.rs:1847` + construction at `:8288-8290`) | `beab904` (revert) | 4102/0 | NEUTRAL −3.6 % (p=0.10) | WIN −3.7 % (p<0.0001) | NEUTRAL −0.6 % (p=0.33) | **LOSS +7.4 %** (p<0.0001) | n/a (test killed during ledger run) | **REJECT** — Welch chain_1000 LOSS p<0.0001. 14 sole-diff outliers (0.002 %) carry load-bearing distinctions. Reverted in-tree. |
| 11 S0 | 2026-05-26 | Per-class Fork breakdown gate instrumentation (`fork_total_by_class: [u64; 4]` + classification in `WpdaStepAction::Fork` arm) | `32f331b` | 4102/0 | n/a | n/a | n/a | n/a | n/a | **ACCEPT (data) + SKIP-AFTER-DATA** — `lex_fork=0 (0.0%)`, `cross_cat_total=100 %`, `avg_fanout=1.67` (below 2.0 threshold). Per Plan agent decision table: combined % > 30 % but avg_fanout ≤ 2.0 → SKIP. Substages 11-S1.a/b/c/d/e CANCELLED. |
| 12 S0 | 2026-05-26 | `binder_scope_marks` + `optional_scope_marks` length histograms | `67d4a3e` (shared with 10 S0) | 4102/0 | n/a | n/a | n/a | n/a | n/a | **ACCEPT (data) + SKIP-BEFORE-ATTEMPT** — all 3 histograms 100 % at length 0 on chain workload (max=0, n=145,054). Path-tree arena for scope marks = wasted effort for chain_10000 closure. |
| 8 S1 | 2026-05-26 | `VisitedSetArena<T>` + walker-global LRU cache (canonical-order path-tree, K=64 default) — STANDALONE module + 18 tests | `db6a4c8` | 4120/0 | n/a | n/a | n/a | n/a | n/a | **ACCEPT** (standalone module; 18/18 tests; retained for potential future reuse). |
| 8 S2 | 2026-05-26 | Wire `BranchCursor::visited_dispatch_id` end-to-end (30+ sites: ctor / clone / Push singleton bucket / B14 per-branch / Fork-arm propagate / cohort shell / commit_winner / stats) | `57152e3` ATTEMPT → reverted at `(HEAD^1)` | 4120/0 | LOSS +3.91 % (p=0.0013, K=64) / +3.91 % (p=0.0001, K=128) | **LOSS +7.57 %** (p<0.0001, K=64) / **+8.39 %** (p<0.0001, K=128) | LOSS +7.41 % / +9.78 % (K=64/128) | LOSS +16.36 % / +13.55 % (K=64/K=128) | n/a (not measured; Welch decisive) | **REJECT** — chain_100 LOSS > 5 % at BOTH K=64 AND K=128 (Plan agent prespecified falsifier triggered). Arena `contains()` costs (cache lookup + LRU touch + FxHashSet lookup) exceed memory savings vs `Arc<FxHashSet>::contains()`. Reverted via `git revert HEAD`. |
| 9 S0 | 2026-05-26 | Cohort revive share confirmation (re-using Exp 11 S0 data) | n/a (data already in ledger) | n/a | n/a | n/a | n/a | n/a | n/a | **ACCEPT** — gate fires: `cohort_cursors_emitted = 335,808` vs `branch_cursors_sum = 193,102` = **174 %** (threshold ≥ 50 %). Proceed to S1.a. |
| 9 S1.a | 2026-05-26 | `CohortContinuation<W>` type + new module `cohort_continuation.rs` + `deferred_continuations` field on `DispatchCacheEntry::{InFlight, Resolved}` (additive, no construction) | `1f26cfa` | 4124/0 (4120 + 4 new tests) | n/a (no construction) | n/a | n/a | n/a | n/a | **ACCEPT** (types-only, additive). |
| 9 S1.b | 2026-05-26 | Dual-write `CohortContinuation` at Sites A (InflightCollision) + B (ResolvedHit) — `try_build_continuation` helper + `push_deferred_continuation` cache helper | pending | pending | pending | pending | pending | pending | n/a | **IN PROGRESS** |
