...
**Formal Welch t-test (2026-05-28, chain_50/100/200 LEFT-assoc wall, hyperfine -N --warmup 5 --runs 30, `taskset -c 2` pinned, pre-Arc `b780295` vs post-Arc):**

| N | pre (ms) | post (ms) | speedup | t | df | p (2-tail) | Cohen's d |
|---|---------:|----------:|--------:|----:|----:|-----------:|----------:|
| 50  | 8.01  | 5.47  | 1.47× | 73.7  | 58.0 | ~0 (<1e-300) | 19.0 |
| 100 | 17.81 | 7.94  | 2.24× | 217.9 | 56.8 | ~0           | 56.3 |
| 200 | 54.02 | 13.02 | 4.15× | 168.1 | 29.7 | ~0           | 43.4 |
...
**Hypothesis (S1):** the §3 pre-fork trigger + §4 right-nested `synth_binary_chain` make `2^2^…^2` absorb in one walker step, parse + eval right-associated, < 500 MB (~160 target), fork suppressed; no regressions. **CONFIRMED.**

**Changes (2 source files; +361/-1):**
- `prattail/src/wpda_walker.rs`: free fn `peek_binary_chain` (O(N) zero-alloc chain recognizer; head atom at `op_pos-1`); methods `synth_atom_symbol` (atom leaf via `(atom_cat<<16)|NumLit` packing — plan-V7 fix, NOT the chart's bare-`outer_rule_idx`-for-atoms bug) + `synth_binary_chain` (whole-chain root spanning `[head_pos, chain_end)`, RIGHT-nested when `spec.assoc_right`, arity-2 packings `[lhs, rhs]` with NO operator child; weight `iter_weight^(m-1)`); `IterativeChainAbsorb` arm dispatches `spec.assoc_right → synth_binary_chain` (pops the head-atom Symbol then pushes the root) else the unchanged `earley_outboard_chain` left-assoc chart path.
- `macros/.../wpda_codegen/engine_impl.rs`: pre-fork trigger after `__cands` is built, before `match __cands.len()` — for the leading infix candidate, if `iter_eligible_<cat>` → `Some(spec)` with `spec.assoc_right` AND `peek_binary_chain(tokens, _pos, 5)`, emit `IterativeChainAbsorb { new_state: Unwinding, spec }` and suppress the fork. LEFT-assoc keeps the singleton path (untouched).

**Empirically-determined post-absorb geometry (traced at WALK-S1 on `2^2^…^2` N=50):** at the InfixLoop dispatch of the FIRST `^`, `cursor.pos = 1` (ON the operator), `chain_start = 0`; the sppf_stack holds EXACTLY ONE node — the head-atom Symbol spanning `[0,1)` (SppfId 2); the GSS frontier_top is the enclosing `CategoryEntry(Int)`. The synthesized whole-chain root spans `[0, 99)` (`root_lo=0, root_hi=99` for 50 atoms = 99 tokens) — matching the chart's whole-chain contract `root_lo=0`. The arm therefore POPs the single head-atom Symbol and PUSHes the root in its place, sets `Unwinding`; with no Return frame pushed, Unwinding-CategoryEntry resolves to Accept. (The absorb fires ~4× per parse from cohort re-entry; all dedup to the SAME interned root — O(N).)
...
**Result:** RIGHT-assoc `^` chains now absorb O(N) (memory + time). `right_assoc_chain_10000` 371 MB → **37.4 MB**. LEFT-assoc chart path untouched (114.8 MB preserved). `earley_outboard_chain` retained for the left-assoc path (NOT made dead). S2 (ternary mixfix pre-fork trigger) and S3 (migrate AddInt onto `synth_binary_chain`) remain.

## C1-M — WALK-S2: pre-fork MIXFIX ternary (`? :`) H3 absorption via direct right-nested-in-else SPPF synthesis (2026-05-29)

**Hypothesis (S2):** `peek_ternary_chain` + a mixfix-tier pre-fork trigger + `synth_ternary_chain` make `0?1:…:0` absorb in one walker step, parse + eval right-nested in the else slot, < 500 MB (~280 target); no regressions (in particular the mixfix-tier change must not perturb other languages' mixfix ops). **CONFIRMED.**

**Why ternary needs its own trigger (D2/V5):** `Tern` is MIXFIX. WALK-S0 made it canonical-eligible (`iter_eligible_int` → `Some(spec)` with `spec.is_mixfix`, `spec.trigger="?"`, `spec.sep=":"`). The binary singleton fast-path and S1's infix pre-fork trigger are both gated `!spec.is_mixfix` / consult `#infix_loop_dispatch` (the `?` token is NOT an infix op — it is matched only by `#mixfix_dispatch`), so `Tern` reaches neither. Mixfix associativity is hard-coded `Left` at classification and `step right` does not propagate; right-recursion is STRUCTURAL (`right_recursive_tail`). So the trigger had to be added at the MIXFIX tier of the InfixLoop dispatch (which otherwise pushes a `MixfixMarker` then a `PrefixDispatch` for the inner operand).

**Changes (2 source files; +396/-14):**
- `prattail/src/wpda_walker.rs`: free fn `peek_ternary_chain` (O(N) zero-alloc recognizer; from the leading `?` at `trigger_pos`, the head cond `c0` is at `trigger_pos-1`; pattern `? t : <atom>` per level where the trailing atom is the NEXT cond if followed by another `?` else `e_final`; operators compared by EXACT `peek_text` against the spec's `trigger`/`sep`, operands validated as "an atom" iff `peek_kind` is `Some` and `peek_text` is neither operator); method `synth_ternary_chain` (whole-chain root spanning `[head_pos, chain_end)`, RIGHT-nested in the else slot, iterative innermost-first: `acc=atom(e_final)`, then per level `pack=packing(T, [atom(c_i), atom(t_i), acc], w)` with EXACTLY 3 children `[cond, then, else]` and NO `?`/`:` separator children; weight `iter_weight^k` for `k` levels); the `IterativeChainAbsorb` arm gains a `spec.is_mixfix` branch (before the `spec.assoc_right` branch) dispatching `synth_ternary_chain` with the SAME post-absorb contract as the binary right-assoc branch (pop the single head-cond Symbol, push the whole-chain root, `cursor.pos=chain_end`, `multiply_cursor_weight`, record `chain_absorbed_intervals`, `Unwinding`, `cursor_resolution_check`). Reuses `synth_atom_symbol` verbatim.
- `macros/.../wpda_codegen/engine_impl.rs`: a SECOND pre-fork trigger after the mixfix tier (after `__cands` is built, before S1's infix pre-fork block) — for the leading mixfix candidate (`#mixfix_dispatch` → `Some((l_bp, result_src, rule_idx))`), if `iter_eligible_<cat>(result_src, rule_idx)` → `Some(spec)` with `spec.is_mixfix` AND `peek_ternary_chain(tokens, _pos, spec.trigger, spec.sep, 2)`, emit `IterativeChainAbsorb { new_state: Unwinding, spec }` and SUPPRESS the fork (the MixfixMarker push is bypassed by the early `return`). Non-Tern mixfix ops in other languages have no `Some(spec)` (the `right_recursive_tail` + exact-shape gate restricts eligibility), so they are bit-identical.

**Empirically-determined post-absorb geometry (traced at WALK-S2 on `0?1:0?1:0?1:0?1:0` N=4 + `1?7:0?9:3` N=2):** at the mixfix-tier dispatch of the FIRST `?`, `cursor.pos = 1` (ON the trigger), `chain_start = 0`; the sppf_stack holds EXACTLY ONE node — the head-cond `c0` Symbol; the GSS frontier_top is the enclosing `CategoryEntry(Int)`. The synthesized whole-chain root spans `[0, chain_end)` (`root_lo=0, root_hi=17` for the N=4 chain of 17 tokens; `root_lo=0, root_hi=9` for the N=2 chains of 9 tokens — `chain_end` lands on the EOF sentinel). The arm POPs the single head-cond Symbol and PUSHes the root, sets `Unwinding`; with no Return frame pushed, Unwinding-CategoryEntry resolves to Accept. IDENTICAL to S1's binary contract. (The absorb fires ~4× per parse from cohort re-entry; all dedup to the SAME interned root — O(N).)
