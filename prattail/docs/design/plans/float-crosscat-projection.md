...
## ⚠ FALSIFICATION — Option E FALSIFIED at M9.1 (R3 STOP), 2026-06-01 — re-localized to the FORWARD cross-cat-dispatch / lex-Fork layer

Option E (Plan-agent-confirmed) was IMPLEMENTED + FALSIFIED. The M9.0 inert diagnostic re-localized AGAIN: the §1.4 "INT collects a span-anchored Pass-1 body, FLOAT doesn't" framing is FALSE — **INT *also* collects no pos-2 span-anchored body** in `take_span_anchored_outer_cast` Pass-1. The real INT↔FLOAT differentiator is the FORWARD `pos_at_dispatch`-keyed cohort resolve+drain (NOT the span-anchored EOI drain): INT resolves many cohort entries at the pos-2 outer-operand dispatch (incl. the Int→BigInt redundancy `wrap:(6,1)`); FLOAT resolves ZERO at pos-2 and has NO Float-source (`src:5`) entry anywhere — its sole ProcFloat reading is consumed by the same-cat FloatBin Fork and **its CrossCatProjection edge is NEVER POPPED.**

**M9.1 (R3, both horns):** E1 (resolve-site promotion) was structurally impossible (the inner Float fold never reaches `resolve()`). E2 (SPPF-arena body scan) WAS implemented + DOES deliver the genuine `[4,8]` Float body to the right paused ProcFloat member (`PAIR R{sym=25,span=[4,8],body_cat=5} | K_sib{pos:2,src:5,wrap:(0,1)}`) — but the spliced cursor STILL never accepts (R3 horn 2; the existing `revive_orphaned_cohort_members_once` fails identically = genuine upstream defect, NOT a bad-snapshot artifact), AND forward-convergence still fails (R3 horn 1: the inner cast's projection edge is never popped under a `float`-outer context). STOPPED per §7 R3 (no force/weight/threshold/DSL). E2 gated DEFAULT-OFF behind `B4_PROJ_DRAIN_ENABLE=1`; the production binary is byte-identical to the base.

**Refined trigger:** the inner cast must produce **Int / Float / Fixed** (the 5-way-ambiguous cast-keyword categories) — `float(uint|bigint|bool|str|sin|+)`, `fixed(float)` all converge GREEN; only `float(int|float|fixed)` fail. The defect is the nesting of two 5-way-ambiguous cast keywords where the inner produces a 5-way-cast-keyword category.

**No regression, base preserved** (calc 214/6, gauntlet 4220/0, op-suites green); `test_nested_float_int_arithmetic` GREEN; the §1.2 isolation matrix preserved. Tag `sigb-b3-M9-R3-stop` (`9eb9a60`).

**Base-count reconciliation (pre-existing, orthogonal):** the authoritative RELEASE base is calc **214/6** (NOT the 213/3 in §0/§5 — a different profile/filter). The 6 = 3 release-only overflow-wrap failures (`test_int_add_overflow_wraps_in_release`, `test_int_pow_31_wraps_in_release`, `test_uint32_add_overflow_wraps_in_release` — calculator promotes `2147483647+1` to BigInt instead of i32-wrapping) + the 2 Float targets + pre-existing `test_bool_from_list_elem`. The 3 overflow-wrap tests are a SEPARATE pre-existing residual.

**NEXT — the 5th layer (the localized TRUE ROOT):** the FORWARD-CONVERGENCE / PROJECTION-POP layer — *why the inner keyword-cast's `CrossCatProjection` edge is never popped under a same-class (`float`-outer, Int/Float/Fixed-inner) context.* One layer ABOVE cohort projection — exactly where M6 (coercion-splice), M7 (span-anchored revival), M8 (Option D dedup), and M9 (cohort projection) each ran out of leverage. The fix likely belongs in the WALKER'S CROSS-CAT DISPATCH / LEX-FORK strategy (how the inner `float`/`int` keyword's 5-way ambiguity resolves under an enclosing same-keyword cast), NOT a drain/cohort mechanism. Artifacts: `/var/tmp/suite-green/cast-family-M9-{R3-FINAL,DELTA-only}.patch`, `float-proj-R3-minimal-reproducer.md`, `float-proj-M90-VERDICT.md`; tag `sigb-b3-M9-R3-stop` (`9eb9a60`).
