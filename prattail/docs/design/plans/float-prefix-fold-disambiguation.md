...
## ⚠ FALSIFICATION — Option D FALSIFIED at M8.1 (R3 STOP), 2026-06-01

Option D (trigger-anchored fold `lo_pos`) was IMPLEMENTED + FALSIFIED. The §1.4 SPPF-dedup mechanism is REAL at the Symbol level but is **NOT the operative blocker**. The M8.0 inert diagnostic re-localized the true root **before any behavioral wiring was committed**; M8.1 then confirmed the falsification and was reverted cleanly (base restored, NO regression, gauntlet 4221/0).

**M8.0 (PASS, inert):** confirmed the §1.4 locus (both inner folds anchor their result Symbol at the operand `lo`, never the keyword, because `consume_trigger:true` discards the keyword) AND that `take_span_anchored_outer_cast` returns 0 Float pairings (re-confirms M7.2). **But the decisive observation re-localizes the defect:** for `int(int(5,32),32)` the inner Int fold span `[4,8]` carries a **Proc projection Symbol** (`ProcInt`, nt=0; ~2057 interns) → the outer `IntBin`'s `a:Proc` slot is satisfiable → forward parse reaches EOF (`hi=11`) → PASSES. For `float(float(10,64),64)` the inner Float fold span carries **only the Float tag** → **no `ProcFloat` projection** → the outer `FloatBin`'s `a:Proc` slot is unsatisfiable → parse dies at the inner `)` (`hi=8`) → FAILS. **The literal §1.4 "inner FloatBin collides with FloatId/ProcFloat over the same span" was NOT observed** — the inner FloatBin Symbol over `[4,8]` is clean. The defect is the operand-anchoring *failing to surface a `ProcFloat` projection*, i.e. a CROSS-CAT-PROJECTION asymmetry, NOT a Symbol-dedup collision.

**M8.1 (FAIL → R3):** the fix fired correctly + symmetrically (keyword-anchored `(Float,lo=2,hi=8)` Symbols interned — dedup-distinction achieved), but (a) **both Float targets STILL FAILED** — no `ProcFloat` projection appears even with anchoring; nothing dispatches a cross-cat cohort at the outer operand pos (all cross-cat resolutions at pos 4/6, none at pos 2); revival Pass-1 still finds 0 Float bodies; (b) it **REGRESSED** `test_nested_float_int_arithmetic` (`3.0 * float(float(10,64),64)`, "premature lex-Fork acceptance" — anchoring perturbed the slack/acceptance arithmetic on a path that DID converge). Closed via NEITHER forward-convergence NOR revival-drain = the R3 STOP condition. Reverted per the guardrail (no force/weight/threshold).

**Retained as inert scaffolding (pure additions, 0 deletions):** the M8.0 `SIGB_FOLD_DEDUP` diagnostics + the `is_keyword_multiarg_fold` predicate + the `B3_FOLD_TRIGGER_DISABLE` lever. `allocate_fork_push_child` has NO trigger emission (reverted); `is_keyword_multiarg_fold` defined but uncalled.

**Baseline note:** this agent measured the Bool-closed base as calc **213/3** (vs the span-anchored agent's 214/6) — a test-count-scope/harness difference to reconcile when the true merge baseline is pinned; the substantive facts (Bool `:2188` closed, the 2 Float targets open) hold on both.

**NEXT — the real generalized fix is a DIFFERENT LAYER (cross-cat projection):** investigate WHY the inner Float fold result fails to project via `ProcFloat` into a Proc-operand context where the inner Int fold projects via `ProcInt`. Candidate loci: the cross-cat cohort dispatch at the outer operand position; the Pass-2a `CrossCatProjection` enumeration (`ProcFloat` `calculator.rs:101` vs `ProcInt` `:100` — are both enumerated/fired identically?); a Float-lexing-boundary interaction; a BP/precedence difference. This is a CROSS-CAT-PROJECTION problem, not a Symbol-dedup problem.

**Artifacts:** `/var/tmp/suite-green/float-fold-FULL-from-6507b9c.patch`, `float-fold-DELTA-over-d2d9a3b.patch` (371 insertions / 0 deletions), `float-prefix-fold-disambiguation-RESULTS.md`; worktree commits `0057256` (M8.0) / `dc27493` (M8.1 falsified+reverted).
