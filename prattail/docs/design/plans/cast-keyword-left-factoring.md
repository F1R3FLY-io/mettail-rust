...
- **M0 — DIAGNOSTIC-CONFIRM gate (BLOCKING; the M*.0 gate the prompt requires FIRST).** Under `systemd-run --user --scope -p MemoryMax=32G`, ONE build at a time:
  ...
  4. Confirm chain/cross-cat neutrality of the predicate: `classify_trigger_group` marks ONLY the 4 mixed cast groups; dump the group classification for calculator + rholang + every test language and assert no chain/operator/projection group is `MixedCastGroup`.
