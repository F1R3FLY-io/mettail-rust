...
BooleanWeight underpins dead-rule detection via a five-tier analysis in
...
|------------------------------|---------------|---------|----------------------------------------------------------|
| Five-tier dead-rule analysis | `pipeline.rs` | 106–207 | `detect_dead_rules()` returns `Vec<DeadRuleWarning>`     |
...
Dead-rule detection uses a five-tier algorithm in `detect_dead_rules()`
(`pipeline.rs:106–207`):
...
The five-tier system covers **all** rule types — no rule is excluded from
...
types (infix, var, literal, cross-category, cast).  The five-tier design
...
- **`detect_dead_rules()` (five-tier)**: `pipeline.rs:106–207`
