...
- [G08](../grammar/G08-missing-cast-to-root.md) -- Detects categories with no **directed** value-flow path to the primary. G08 and A4 (W01 Tier 4) are **complementary**: G08 preserves direction across cast rules, cross-category rules, and syntax embeddings, while A4 uses a richer undirected graph including syntax-level references. G08 can still flag categories that are mentioned structurally but cannot produce values for the primary result.
