...
fn generate_parser_code(
    ...
    // ── DEFECT-B (amendments 3 + 8): fleet FOLLOW A/B cross-check (env-gated) ──
    // When `PRATTAIL_FOLLOW_AB` is set, recompute FOLLOW via BOTH the incremental
    // worklist and the order-independent reference `compute_follow_sets_from_inputs`,
    // then assert per-category set-equality and emit a per-grammar receipt to
    // stderr. Building `mettail-languages` with this env set turns the whole
    // 22-grammar fleet into a single incremental-equals-reference A/B: it makes
    // "the other 20 languages are unchanged" ANALYTIC (any grammar whose
    // incremental FOLLOW diverges from the reference surfaces here as MISMATCH),
    // and the incr/ref wall times (mean over a fixed repeat, to amortize
    // measurement overhead) MEASURE the DB01 build-time saving. This block is
    // pure instrumentation — the downstream `follow_sets` binding is untouched —
    // and costs nothing when the env var is unset (the reference pass never runs).
    if std::env::var_os("PRATTAIL_FOLLOW_AB").is_some() {
        use std::time::Instant;
        // Fixed internal repeat: FOLLOW is O(passes×rules) and sub-microsecond at
        // ≤20 categories, so single-shot timing is pure measurement noise.
        const AB_ROUNDS: u32 = 256;

        let (incr_follow, _) = compute_follow_sets_incremental(
            &bundle.follow_inputs,
            &category_names,
            &first_sets,
            primary_category,
        );
        let ref_follow = compute_follow_sets_from_inputs(
            &bundle.follow_inputs,
            &category_names,
            &first_sets,
            primary_category,
        ...
        for cat in &category_names {
            let mut incr_tokens: Vec<String> = incr_follow
                .get(cat)
                .map(|fs| fs.tokens.iter().cloned().collect())
                .unwrap_or_default();
            let mut ref_tokens: Vec<String> = ref_follow
                .get(cat)
                .map(|fs| fs.tokens.iter().cloned().collect())
                .unwrap_or_default();
            incr_tokens.sort();
            ref_tokens.sort();
            if incr_tokens != ref_tokens {
                mismatches
                    .push(format!("{cat}{{incr={incr_tokens:?} ref={ref_tokens:?}}}"));
            }
        }

        let t_incr = Instant::now();
        for _ in 0..AB_ROUNDS {
            let _ = compute_follow_sets_incremental(
                &bundle.follow_inputs,
                &category_names,
                &first_sets,
                primary_category,
        ...
        let incr_ns = t_incr.elapsed().as_nanos() / AB_ROUNDS as u128;

        let t_ref = Instant::now();
        for _ in 0..AB_ROUNDS {
            let _ = compute_follow_sets_from_inputs(
                &bundle.follow_inputs,
                &category_names,
                &first_sets,
                primary_category,
        ...
        let ref_ns = t_ref.elapsed().as_nanos() / AB_ROUNDS as u128;

        eprintln!(
            "[FOLLOW-AB] {name}: {status} cats={ncats} incr={incr_ns}ns ref={ref_ns}ns{detail}",
            name = bundle.grammar_name,
            status = if mismatches.is_empty() { "MATCH" } else { "MISMATCH" },
            ncats = category_names.len(),
