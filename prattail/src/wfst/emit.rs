...
///
/// DEFECT-B HAZARD NOTE: this function is DEAD in production — its only callers
/// are unit tests (`wfst/tests.rs`). Its `token_groups` (`HashMap<TokenId, …>`)
/// iteration in the per-token comment emission below is hasher-incidental, so IF
/// this were ever wired into `parser.rs` emission it would reintroduce the B-M2
/// class of nondeterminism (unordered rows in generated bytes). Sort
/// `token_groups` by `TokenId` (or key it with a `BTreeMap`) before reviving it
/// as a real emitter.
