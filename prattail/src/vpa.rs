...
//!   and determinization. Equivalence and inclusion are decidable; for
//!   nondeterministic VPAs, determinization makes their worst-case complexity
//!   exponential.
...
//! **Severity**: Warning
//!
//! Fires when a terminal has conflicting call/return/internal roles, so the
//! alphabet is not a valid visibly pushdown partition.
//!
//! ### V06 — Exact-Decision Complexity Risk
//!
//! **Severity**: Warning
//!
//! Warns when a large structurally invalid VPA cannot enter the exact Boolean
//! decision pipeline. It is not an inclusion verdict. Arbitrary-semiring
//! quantitative inclusion is not exposed: idempotence alone does not make
//! weighted VPA inclusion decidable.
...
#[path = "vpa_decision.rs"]
mod decision;
pub use decision::VpaValidationError;
...
mod decision_semiring_sealed {
    pub trait Sealed {}
}

/// Semirings for which PraTTaIL exposes exact VPA language decision procedures.
///
/// Weighted VPA determinization and inclusion are not available for arbitrary
/// semirings. The current proven scope is Boolean support-language semantics.
///
/// This trait is sealed: downstream crates cannot extend the proof boundary by
/// implementing the marker for an unverified weight domain.
///
/// ```compile_fail
/// use std::collections::HashSet;
///
/// use mettail_prattail::automata::semiring::TropicalWeight;
/// use mettail_prattail::vpa::{VpaAlphabet, WeightedVpa};
///
/// let alphabet = VpaAlphabet::new(HashSet::new(), HashSet::new(), HashSet::new());
/// let weighted = WeightedVpa::<TropicalWeight>::new(alphabet);
/// let _unsupported = weighted.determinize();
/// ```
pub trait VpaDecisionSemiring: IdempotentSemiring + decision_semiring_sealed::Sealed {}

impl decision_semiring_sealed::Sealed for BooleanWeight {}
impl VpaDecisionSemiring for BooleanWeight {}
...
impl<W: Semiring> WeightedVpa<W> {
    ...
    /// Determinize this VPA using the Alur–Madhusudan summary construction.
    ///
    /// ## Algorithm
    ///
    /// - Each deterministic state stores a relation `S ⊆ Q × Q` of
    ///   well-matched summaries and a reachable-state set `R ⊆ Q`.
    /// - Call stack symbols retain the caller summary state and call symbol,
    ///   preserving the call/push/return correlation.
    /// - Returns at the bottom read the original bottom marker without popping
    ///   the generated bottom marker.
    ...
    where
        W: VpaDecisionSemiring,
    {
        determinize_impl(self)
    }

    /// Validate the public VPA representation before exact analysis.
    pub fn validate(&self) -> Result<(), VpaValidationError> {
            ...
            match self.alphabet.classify(sym) {
                ...
                Some(SymbolKind::Return) => {
                    for ((state, stack), weight) in &configs {
                        let top = stack
                            .last()
                            .expect("weighted VPA configurations always retain a bottom marker");
                        if let Some(targets) =
    ...
    /// Determinize Boolean support-language semantics exactly.
    ///
    /// Arbitrary-semiring weighted determinization is not generally defined.
    /// The `VpaDecisionSemiring` bound exposes this method only for semirings
    /// with a registered, proven decision procedure (currently Boolean).
    ...
    /// Decide support-language inclusion exactly.
    ///
    /// This API is intentionally restricted by `VpaDecisionSemiring`.
    /// For the current Boolean implementation, it decides
    /// `L(self) ⊆ L(other)` with exact VPA complement, intersection, and
    /// matched-summary emptiness—without a stack-depth cap.
    pub fn weighted_inclusion(&self, other: &WeightedVpa<W>) -> bool
    where
        W: VpaDecisionSemiring,
    {
        let left = decision::boolean_support(self);
        let right = decision::boolean_support(other);
        check_inclusion(&left, &right)
    }
}
...
pub fn check_equivalence(a: &Vpa, b: &Vpa) -> bool {
    check_inclusion(a, b) && check_inclusion(b, a)
}

/// Decide Boolean VPA language inclusion exactly.
pub fn check_inclusion(a: &Vpa, b: &Vpa) -> bool {
    a.validate()
        .unwrap_or_else(|error| panic!("invalid left VPA passed to inclusion: {error}"));
    b.validate()
        .unwrap_or_else(|error| panic!("invalid right VPA passed to inclusion: {error}"));

    // Complement must be total over the union alphabet. Merely merging the
    // alphabets after complement would mishandle symbols absent from `b`.
    let alphabet = merge_alphabets(&a.alphabet, &b.alphabet);
    let mut aligned_a = a.clone();
    aligned_a.alphabet = alphabet.clone();
    let mut aligned_b = b.clone();
    aligned_b.alphabet = alphabet;

    // L(A) ⊆ L(B) iff L(A) ∩ L(B̄) = ∅.
    let b_complement = complement(&aligned_b);
    let difference = intersect(&aligned_a, &b_complement);
    is_language_empty(&difference)
}

/// Validate and decide whether a VPA accepts the empty language.
///
/// The decision procedure computes the least finite relation of well-matched
/// state summaries, then performs ground-level and above-bottom reachability.
/// It is exact and has no arbitrary stack-depth bound.
pub fn try_is_language_empty(vpa: &Vpa) -> Result<bool, VpaValidationError> {
    decision::is_language_empty(vpa)
}

/// Decide whether a valid VPA accepts the empty language.
///
/// # Panics
///
/// Panics when the public VPA representation violates structural invariants.
/// Use `try_is_language_empty` when invalid input must be reported as data.
pub fn is_language_empty(vpa: &Vpa) -> bool {
    try_is_language_empty(vpa)
        .unwrap_or_else(|error| panic!("invalid VPA passed to is_language_empty: {error}"))
}
...
/// Determinize a VPA with the standard summary-state construction.
///
/// Each state contains a well-matched relation `S ⊆ Q × Q` and a reachable
/// set `R ⊆ Q`. Generated stack symbols retain both caller state and call
/// symbol, so matched returns preserve the source call/push/return correlation.
/// The output is deterministic and total over its declared alphabet.
fn determinize_impl<W: VpaDecisionSemiring>(vpa: &WeightedVpa<W>) -> WeightedVpa<W> {
    decision::determinize(vpa)
        .unwrap_or_else(|error| panic!("invalid VPA passed to determinize: {error}"))
}
...
pub fn intersect(a: &Vpa, b: &Vpa) -> Vpa {
    a.validate()
        .unwrap_or_else(|error| panic!("invalid left VPA passed to intersect: {error}"));
    b.validate()
        .unwrap_or_else(|error| panic!("invalid right VPA passed to intersect: {error}"));
    ...
    product.initial_stack_symbol = "\0prattail:vpa:product-bottom".to_string();
    ...
    for &qa in a
        .initial_states
        .iter()
        .filter(|state| decision::active_initial(a, **state))
    {
        for &qb in b
            .initial_states
            .iter()
            .filter(|state| decision::active_initial(b, **state))
        {
            let pid = product.add_state(None);
            pair_to_id.insert((qa, qb), pid);
            product.initial_states.insert(pid);
            if decision::active_accepting(a, qa) && decision::active_accepting(b, qb) {
    ...
    while let Some((qa, qb)) = worklist.pop_front() {
        ...
        for sym in &int_syms {
            ...
            let b_targets = b
                ...
                .cloned()
                .unwrap_or_default();
            for &(ta, wa) in &a_targets {
                if wa.is_zero() {
                    continue;
                }
                for &(tb, wb) in &b_targets {
                    if wb.is_zero() {
                        continue;
                    }
        ...
        for sym in &call_syms {
            ...
            let b_targets = b
                ...
                .cloned()
                .unwrap_or_default();
            for &(ta, ref gamma_a, wa) in &a_targets {
                if wa.is_zero() {
                    continue;
                }
                for &(tb, ref gamma_b, wb) in &b_targets {
                    if wb.is_zero() {
                        continue;
                    }
                    let next_pid = get_or_create_product_state(
                    ...
                    let product_gamma = product_stack_symbol(gamma_a, gamma_b);
                    product
                        .call_transitions
                        .entry((current_pid, sym.clone()))
                        .or_default()
        ...
        for sym in &ret_syms {
            ...
            for (s, _, gamma) in a.return_transitions.keys() {
                if *s == qa {
                    a_gammas.insert(gamma.clone());
                }
            }
            for (s, _, gamma) in b.return_transitions.keys() {
                if *s == qb {
            ...
            for ga in &a_gammas {
                ...
                for gb in &b_gammas {
                    ...
                    let product_gamma =
                        if ga == &a.initial_stack_symbol && gb == &b.initial_stack_symbol {
                            product.initial_stack_symbol.clone()
                        } else {
                            product_stack_symbol(ga, gb)
                        };
                    for &(ta, wa) in &a_targets {
                        if wa.is_zero() {
                            continue;
...
fn product_stack_symbol(left: &str, right: &str) -> String {
    format!("\0prattail:vpa:product-stack:{}:{left}:{}:{right}", left.len(), right.len())
}
...
/// * `classify` — closure that retains both delimiter direction and pair identity.
///
/// # Examples
///
/// ```rust
...
/// let table = build_skip_table(&tokens, |tok| match *tok {
///     "(" => DelimiterClass::Open('('),
...
pub enum DelimiterClass<K> {
    /// Opening delimiter carrying the identity of its matching pair.
    Open(K),
    /// Closing delimiter carrying the identity of its matching pair.
    Close(K),
    /// Token that does not participate in delimiter pairing.
    Internal,
}
    ...
    K: Eq,
{
    ...
    let mut stack: Vec<(usize, K)> = Vec::new();
    for (i, (tok, _)) in tokens.iter().enumerate() {
        match classify(tok) {
            DelimiterClass::Open(kind) => stack.push((i, kind)),
            DelimiterClass::Close(kind) => {
                if stack
                    .last()
                    .is_some_and(|(_, open_kind)| open_kind == &kind)
                {
                    let (opener, _) = stack.pop().expect("stack was just observed as non-empty");
                    table[opener] = Some(i);
                }
            },
...
mod tests {
    ...
    fn simulate(vpa: &Vpa, input: &[&str]) -> bool {
        ...
        for sym in input {
            ...
            match vpa.alphabet.classify(sym) {
                ...
                Some(SymbolKind::Return) => {
                    for (state, stack) in &configs {
                        let top = stack.last().expect("test configuration retains bottom");
                        if let Some(targets) =
                            vpa.return_transitions
                                .get(&(*state, sym_str.clone(), top.clone()))
                        {
    ...
    #[test]
    fn weighted_determinize_boolean_support() {
        // The exact weighted API is intentionally scoped to Boolean support.
        let alpha = VpaAlphabet::new(
            HashSet::new(),
            HashSet::new(),
            ["a".to_string()].into_iter().collect(),
        );
        let mut wvpa = Vpa::new(alpha);
        ...
        // Two Boolean alternatives on the same symbol.
        wvpa.internal_transitions
            .insert((q0, "a".to_string()), vec![(q2, BooleanWeight::one())]);
        wvpa.internal_transitions
            .insert((q1, "a".to_string()), vec![(q3, BooleanWeight::one())]);
    ...
    fn classify_test_delimiter(tok: &str) -> DelimiterClass<char> {
        match tok {
            "(" | ")" => match tok {
                "(" => DelimiterClass::Open('('),
                _ => DelimiterClass::Close('('),
            },
            "{" | "}" => match tok {
                "{" => DelimiterClass::Open('{'),
                _ => DelimiterClass::Close('{'),
    ...
    fn test_skip_table_simple() {
        ...
        let table = build_skip_table(&tokens, |tok| classify_test_delimiter(tok));
    ...
    #[test]
    fn skip_table_never_pairs_different_delimiter_kinds() {
        let tokens = vec![("(", test_range(0)), ("]", test_range(1))];
        let table = build_skip_table(&tokens, |tok| classify_test_delimiter(tok));
        assert_eq!(table, vec![None, None]);
    }

    #[test]
    fn mismatched_close_does_not_destroy_a_later_valid_pair() {
        let tokens = vec![
            ("(", test_range(0)),
            ("[", test_range(1)),
            (")", test_range(2)),
        ...
        let table = build_skip_table(&tokens, |tok| classify_test_delimiter(tok));
        assert_eq!(table, vec![Some(4), Some(3), None, None, None]);
    }

    #[test]
    fn valid_nesting_is_not_bounded_by_vpa_state_count() {
        let vpa = construct_vpa(&[("(".to_string(), ")".to_string())], &[]);
        let depth = vpa.num_states() * 8 + 1;
        let mut word = vec!["("; depth];
