...
impl<W: SemiringRef> DispatchCohortCache<W> {
    ...
    /// Phase F.13 chain_10000 Exp 9 Substage 1.b (2026-05-26): push a
    /// `CohortContinuation` into the matched entry's
    /// `deferred_continuations` Vec. Mirrors `pause_cohort_member`'s
    /// two-arm match pattern (`InFlight` + `Resolved`).
    ///
    /// Returns `true` if the continuation was pushed (entry exists and
    /// is `InFlight` or `Resolved`, and the cap has room), `false`
    /// otherwise (entry missing, `Failed`, or cap reached).
    ///
    /// Hard cap `MAX_DEFERRED_PER_KEY = 64` (`cohort_continuation.rs:71`)
    /// — pathological grammars overflow gracefully back to per-cursor
    /// revive (the dual-write design keeps that path live).
