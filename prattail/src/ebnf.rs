...
pub fn write_ebnf_output(ebnf: &str, language_name: &str, dump_target: &str) {
    ...
    if let Err(e) = std::fs::create_dir_all(&dir) {
        eprintln!(
            "warning: PRATTAIL_DUMP_EBNF: failed to create directory {:?}: {}",
            dir, e
        );
        return;
    }
    ...
    match std::fs::write(&path, ebnf) {
        ...
        Err(e) => eprintln!(
            "warning: PRATTAIL_DUMP_EBNF: failed to write {:?}: {}",
            path, e
        ),
    }
}
...
fn write_precedence_table(
    ...
    let mut infix_ops: Vec<&&InfixOperator> = all_ops
        .iter()
    ...
    // Postfix operators
    let mut postfix_ops: Vec<&&InfixOperator> = all_ops
        .iter()
        .filter(|op| op.is_postfix)
        .collect();
...
fn write_category(
    ...
    if cat.is_primary && has_binders {
        ...
        productions.push((
            "\"^\" \"[\" <ident> { \",\" <ident> } \"]\" \".\" \"{\" Cat \"}\"".replace("Cat", cat_name),
...
fn format_syntax_item(item: &SyntaxItemSpec) -> String {
    match item {
        ...
        SyntaxItemSpec::Collection {
            ...
            format!(
                "{{ {} / \"{}\" }}  (* {} *)",
                element_category, separator, kind_str
            )
        }
...
mod tests {
    ...
    fn test_ebnf_calculator_snapshot() {
        ...
        assert!(
            ebnf.contains("Generated by PraTTaIL"),
            "should contain generator attribution"
        );

        // Lexical tokens
        assert!(
            ebnf.contains("<integer>"),
            "should contain integer token: {}",
            ebnf
        );
        assert!(
            ebnf.contains("<ident>"),
        ...
        assert!(
            ebnf.contains("Int ="),
            "should contain Int production: {}",
            ebnf
        );

        // Rule labels in annotations
        assert!(
            ebnf.contains("NumLit"),
            "should contain NumLit label: {}",
            ebnf
        );
        assert!(
            ebnf.contains("Add"),
            "should contain Add label: {}",
            ebnf
        );
        assert!(
            ebnf.contains("Mul"),
            "should contain Mul label: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_contains_precedence_table() {
        ...
        assert!(
            ebnf.contains("BP"),
            "should contain BP column header: {}",
            ebnf
        );
        assert!(
            ebnf.contains("Assoc"),
            "should contain Assoc column header: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_contains_first_follow() {
        ...
        assert!(
            ebnf.contains("FIRST(Int)"),
            "should contain FIRST set: {}",
            ebnf
        );
        assert!(
            ebnf.contains("FOLLOW(Int)"),
            "should contain FOLLOW set: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_binder_notation() {
        ...
        assert!(
            ebnf.contains("^x:Term"),
            "should contain binder notation ^x:Term: {}",
            ebnf
        );
        assert!(
            ebnf.contains("binder"),
            "should contain binder annotation: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_collection_notation() {
        ...
        assert!(
            ebnf.contains("Vec"),
            "should contain Vec kind annotation: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_postfix_annotation() {
        ...
        assert!(
            ebnf.contains("postfix"),
            "should contain postfix annotation: {}",
            ebnf
        );
        assert!(
            ebnf.contains("Fact"),
            "should contain Fact label: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_right_assoc_annotation() {
        ...
        assert!(
            ebnf.contains("right"),
            "should contain right-assoc annotation: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_mixfix_annotation() {
        ...
        assert!(
            ebnf.contains("mixfix"),
            "should contain mixfix annotation: {}",
            ebnf
        );
        assert!(
            ebnf.contains("Tern"),
            "should contain Tern label: {}",
            ebnf
        );
    }
    ...
    fn test_ebnf_grouping_annotation() {
        ...
        assert!(
            ebnf.contains("group"),
            "should contain group annotation: {}",
            ebnf
        );
        assert!(
            ebnf.contains("\"(\" Int \")\""),
            "should contain grouping syntax: {}",
            ebnf
        );
    }
}
