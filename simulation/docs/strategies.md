...
These are generated by the `mettail-macros` crate's `generate_strategies()` function at compile time, one per category in the language definition.

For the `SimulationRunner`, any `Strategy<Value = String>` can be passed to `run_campaign()`:

```rust
let strategy = arb_proc(20);  // generated for the RhoCalc language
