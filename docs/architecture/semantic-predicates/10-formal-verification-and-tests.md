...
| Two decidable atom theories over a proved-exhaustive shared universe **combine** via exact joint search — the Nelson-Oppen base case — yielding an EBA with sound/complete satisfiability. | `TheoryCombination.v` | `combined_eba_laws`; `csat_sound`/`csat_complete`, `cwit_sound`/`cwit_total` | `rocq-symbolic-algebra` |
