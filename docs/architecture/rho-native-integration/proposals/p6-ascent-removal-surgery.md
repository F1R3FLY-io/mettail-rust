...
- CHECKPOINT: `cargo check -p mettail-macros` then `cargo check -p mettail-languages` (default features).
...
- `prattail/Cargo.toml`: delete `ascent-parallel`. `mettail-rho-runtime/Cargo.toml`: delete `oracle-ascent`.
...
- `mettail-rho-runtime/Cargo.toml`: delete `[[test]] rho_vs_ascent`, `[[test]] rho_language_backend_report`.
...
- Delete `mettail-rho-runtime/tests/{rho_vs_ascent,rho_language_backend_report}.rs`.
...
- FINAL CHECKPOINT: `cargo check --workspace --all-targets`; `cargo build --workspace`; `cargo tree -p mettail-languages | grep -i ascent` shows ONLY `ascent_syntax_export`.
