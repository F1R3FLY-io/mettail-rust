...
| [20 — Rholang Runtime Backend](20-rholang-runtime-backend.md) | How does the whole backend run — the three layers (matching / firing / congruence), the reflected-`EList` ABI, the `loc:`/`col:`/`cap:`/`sa:`/`eq:`/`ac:` channel scheme, the firing paths (including the A-S5 quiescence driver for the flipped Lambda/Ambient exec), the fail-closed install gate, and metering — all on F1r3node's RhoRuntime / RSpace? |
...
| [26 — In-Rho AC-Family Reference](26-in-rho-ac-family-reference.md) | How is the associative-commutative family rebuilt from scratch — the Scheme-B spread re-sourcing, the site-keyed carrier, AC4 (`HashSet` / `HashMap` / `Zip`), and the Ambient fragment's Cardelli–Gordon alignment record? |
...
| [30 — Retained FLT Candidate Matching](30-retained-flt-candidate-matching.md) | How does RSpace match reflected Foreign Language Term receive patterns through a retained Dovetail automaton and flat stack-safe PDA, while preserving exact spatial-matcher behavior and delegating unsupported shapes verbatim? |
