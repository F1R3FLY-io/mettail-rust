...
| **`loc:` / `col:` / `cap:` / `sa:` / `eq:` / `ac:` channels** | The channel-kind scheme the spread and receivers rendezvous on. Production positional `loc:`/`col:`/`cap:` names come from one `SubjectLocationIndex` and encode `(root site, fixed-width arena position)` exactly (`rho_net_location.rs`), avoiding repeated absolute paths. `loc:` carries head tags, `col:` a node's chain-collapse value read once by its parent, and `cap:` the capture-collapse value read once by a Var-leaf state. `sa:` is a $`\sigma`$-receiver's source/dispatch channel (the accept target). `eq:` is a non-linear consistency (name-equality) guard channel (`RhoNetChannelKind::Consistency`). `ac:` is a site-keyed AC operand-bag carrier ([26](26-in-rho-ac-family-reference.md)). The exported path-string helpers remain compatibility/E-6a utilities, not this production ABI. |
