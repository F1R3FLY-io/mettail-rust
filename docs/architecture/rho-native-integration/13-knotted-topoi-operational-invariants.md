...
|---|---|---|---|---|
| INV-1 | **Injective location channels.** Distinct term locations map to distinct channels: $`c(\ell) = \ulcorner\ell\urcorner`$, injective. | `Definition` "location channels", §5.2 | `SubjectLocationIndex` assigns exact integer positions once; `compact_position_channel` length-prefixes the root site and encodes the position at fixed width, with no digest (`rho_net_location.rs`). `IndexedSubjectChannels.v` proves tuple and channel injectivity. | Satisfied |
