...
| Use PlantUML diagrams where appropriate and colorize them. | PlantUML diagrams appear in [README](README.md), [End-to-End Architecture](02-end-to-end-architecture.md), [Rho-Native Dataflow Lowering](04-rho-native-dataflow-lowering.md), [RSpace Parallel Scheduling](05-rspace-parallel-scheduling.md), [Correctness and Coverage](06-correctness-and-coverage.md), and [Verification and Rollout](07-verification-and-rollout.md). Each diagram assigns colors to participants, states, or proof nodes. |
...
| rendered diagrams | Every PlantUML block has matching `figures/*.puml` source and a non-empty rendered `figures/*.svg` image with an SVG root and closing tag. |
