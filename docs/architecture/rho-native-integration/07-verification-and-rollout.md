...
carries the fingerprint of the `LanguageDef` it lowered. Production
Dovetail/Rho installation additionally wraps the Dovetail compiler and Rho
invocation compiler in typed stages carrying the same fingerprint. A scalar
fragment plan may still serve as oracle evidence, but it cannot be installed as
the production backend for a full generated language whose expanded
`LanguageDef` differs. Once installed, the wrapper advertises `RhoMachine` as
the default runtime backend through the concrete
