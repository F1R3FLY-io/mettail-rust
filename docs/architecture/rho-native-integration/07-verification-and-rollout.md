...
The wrapper is constructed only when the
`RhoDefaultBackendPlan` records the same `LanguageDef` name as `L.name()`;
cross-language plan installation fails before `RhoMachine` can be advertised as
the default. Once installed, the wrapper advertises `RhoMachine` as the default
