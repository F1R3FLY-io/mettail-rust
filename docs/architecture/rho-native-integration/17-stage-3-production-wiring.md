...
> **Status: complete for base rewrites (SwapDemo).** Stage 1 (docs [15](15-in-rho-set-automaton-matching.md)/[16](16-in-rho-verification-plan.md)) built the in-Rho matching *mechanism*; Stage 3 makes it the **actual default backend** a language runs. Base-rewrite matching now executes ON the Rholang interpreter — the internal `sa:` inspection COMMs are unobservable `$\tau$` steps and the rewrite fires as one `c(\ell)` COMM — as the production `run_backend_report(RhoMachine, …)` path, `O1`-optimal and formally verified (FV Phases A/B/C + (ix), all zero-admission).
...
## 8. What runs where, and what is next
...
Remaining stages: **2** (non-linear `eq:` receivers), **AC** (in-Rho AC matching), **3a** (contextual atomic joins, where FV (vii)/(viii) bite), **3b–3f** (RhoCalc `Comm` / Lambda β / Ambient / native), **4** (encoder boundary + adaptive `L` + whole-term spreading), **5** (capstone (v) — whole-`$\llbracket G \rrbracket$` `opcorr`).
