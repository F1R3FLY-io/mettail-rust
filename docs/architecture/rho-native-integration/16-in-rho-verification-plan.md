...
| Stage AC match + fire | done | the HashBag AC receiver runs ON the interpreter — a bag reflects to a process-`Par` soup (`reflect_ground_term_par`; corrected from `EList`, whose `fold_match` is positional), the connective `ac_bag_pattern` matches it ORDER-INDEPENDENTLY in the production `spatial_matcher_pda::ListMachine` (with `sub_pars` supplying remainders), and `ac_sigma_receiver_par` fires $`[\![ R ]\!] \sigma`$ on the dynamic out. Runtime tests `ac_bag_pattern_matches_the_process_soup_in_rho` + `ac_receiver_fires_the_matched_element_on_the_dynamic_out` on the live reducer |
