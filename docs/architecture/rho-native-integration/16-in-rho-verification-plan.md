...
| Stage 3 P5 default-wire | next | the generated `rho_net_match_invocation_from_dovetail_to` + `swapdemo_backed()` capability-gated default backend + the `run_backend_report` integration test — the last Stage 3 piece (P4 subsumed above) |
