...
|---|---|---|---|
| location-channel injectivity | `def:loc` — "distinct locations give distinct channels by injectivity of $`\ulcorner\cdot\urcorner`$" | `site_inj`, Section Hypothesis (`InRhoSameCLTSWeakBisim.v:56`) | the compact subject-position ABI — one shared index assigns exact positions, and the channel encodes the root-site byte length plus a fixed-width position without hashing (`rho_net_location.rs`; `IndexedSubjectChannels.v`; INV-1 in [13 §5](13-knotted-topoi-operational-invariants.md)) |
...
| binder-float completeness — **a theorem now, demoted from premise** | `con:functor` — the congruence action must expose every redex the equations relate | `float_nf_exposes_redexes_in` / `float_nf_exposes_redexes_open` + freshening totality + NewComm-permutation redex invariance (`BinderFloatCanonicalization.v`), proven over the Cardelli–Gordon subset with the AM-2 bag-flatness obligation; **A-S5.8**: the same exposure re-proved over the NAMELESS ABI — `float_exposes_redexes_{in,open,out}` + `float_preserves_bag_flatness` (`InRhoFloatCanonicalization.v`) | **A-S5.8: canonicalization runs PER ITERATION IN-RHO** — the generated `^float` receiver family (`rholang-codegen/src/rho_net_float.rs`: the dispatcher + the equation-derived `^float-hoist:{C}` / `^float-merge:{op}` satellites) floats the S2 seed and EVERY fired contractum on the live reducer (decision Q-AB = A), so a contractum-introduced $`\nu`$ cannot hide a redex from the driver's Match arms; the A-S5.4a host float ([26 §13](26-in-rho-ac-family-reference.md#13-the-ambient-fragment-cardelligordon-alignment)) is RETAINED as defense-in-depth |
| the boundary-float premise ("contracta create no float-hidden redexes") — **discharged constructively (A-S5.8)** | `con:functor` — the congruence action must keep exposing redexes ACROSS firings, not only at injection | `drive_with_float_on_raw_eq_drive_on_canonical` + the ν-hidden-contractum witness `nu_hidden_contractum_redex_fires` with its load-bearing contrast `raw_contractum_is_not_a_redex`, and `float_phase_conservative` (landed bag-driver theorems transfer as corollaries) — `InRhoDriveWithFloat.v`; `float_step_sound` (= `fstep_side_condition` + `float_reachable`) with the C-G side condition held by the SHIFT-IMAGE argument `hoist_side_condition_by_shift_image`, plus the F8-AM-4 split `float_functional_up_to_NewComm` / `float_identity_on_canonical` and the F8-AM-3 lemma `redex_invariant_under_run_permutation` — `InRhoFloatCanonicalization.v` | the drive's firing emission routes every contractum through `⌜^float⌝` before the re-drive; the F8-AM-1 `AcReconstructTemplate::Binder` carrier pre-shifts under-template-binder σ slots by the depth-`k` `^shift(Z,·)` chain (F8-AM-1c); runtime witness: `rholang-runtime/tests/rho_net_ambient_float.rs` (the `Seal` witness fires `{Seal, OpenRule}` with no host float anywhere) |
| host-float display ordering (NewComm run order) | none — a DISPLAY-canonicality convenience, not a KT clause (decision Q-NC) | `float_functional_up_to_NewComm` (the float NF is unique UP TO the NewComm run permutation) + `redex_invariant_under_run_permutation` (exposure is run-order-invariant) — `InRhoFloatCanonicalization.v` | in-Rho NewComm reordering is DELIBERATELY OMITTED: the host's canonical run order is the α-canonical (FIX-A) semantic-KEY minimization (`FramedSemanticKeyHasher`, `binder_congruence.rs`) — a function of the α-quotient the erased ABI retains, but with NO Match-expressible total order or hash over reflected `Par`s — and exposure needs no order; the retained host float is therefore LOAD-BEARING for the run-order-sensitive α exec goldens (F8-AM-5b), display-side only |
...
| moniker `UniqueId` width (AM-6d) — **narrowed to HOST paths (A-S5.8)** | `rem:fresh` — freshness by quoting, transcribed at the HOST float layer's freshener ONLY | stated here as an honest premise; the id allocator is not modeled in Rocq. The IN-RHO float needs NO freshness supply at all: over the nameless ABI, capture avoidance is the SHIFT-IMAGE argument (`hoist_side_condition_by_shift_image` / `fstep_side_condition`, `InRhoFloatCanonicalization.v`) — the extruded binder's index is never in a shift's image, so the premise does not apply on the drive path | moniker's `UniqueId` is a process-global `AtomicUsize` narrowed usize-to-u32 at construction (noted in moniker's source); id recycling would require $`2^{32}`$ freshenings within one process before any collision is possible — relevant now only to the HOST float (Dovetail / display / the retained defense-in-depth boundary pass), never to the per-iteration in-Rho `^float` family |
...
| `InRhoFloatCanonicalization.v` (A-S5.8) — `hoist_side_condition_by_shift_image`, `fstep_side_condition`, `float_reachable` (together: `float_step_sound`), `float_functional_up_to_NewComm` + `float_identity_on_canonical` (the F8-AM-4 split), `redex_invariant_under_run_permutation` (F8-AM-3), `float_preserves_bag_flatness`, `float_exposes_redexes_{in,open,out}` | `con:functor` (equations as structural congruence, now enacted IN-RHO per iteration) + `rem:fresh` (discharged by shift arithmetic, not gensym) | the in-Rho `^float` receiver family over the NAMELESS de Bruijn ABI (run-length configurations): every float step is a C-G composite whose freshness side condition is held by the SHIFT-IMAGE argument; any peel order lands in the same NewComm class (schedule confluence with an explicit two-sided-inverse renaming — the Q-NC record); redexes are exposed iff syntactically present in the float NF |
| `InRhoDriveWithFloat.v` (A-S5.8) — `ffloat_canonical`/`ffloat_idempotent`, `drive_with_float_on_raw_eq_drive_on_canonical`, `nu_hidden_contractum_redex_fires` + `raw_contractum_is_not_a_redex`, `float_phase_conservative` + corollaries | `app:desugar` "A whole GSLT" (the drive loop's per-iteration canonicalization) + `ob:opcorr` | the DRIVER FLOAT PHASE: `d_redex_fire` routes through the float (decisions Q-AB = A, Q-SEED = S2), so driving a RAW value is EXACTLY driving its canonical form — THE PREMISE DISCHARGE — with the Seal-shaped ν-hidden-contractum witness (raw contractum matches NO rule; the float exposes it and it fires) and the conservative embedding under which every landed bag-driver theorem is a corollary |
...
| the enforcement campaign that closed the production deviations | **A-S2 through A-S6 are LANDED**: A-S2 lazy report + static gate + ruleset memoization (the D-stage demotion — report checked $`\iff`$ deferral taken); A-S3 native dispatch as metered system processes; A-S4 lowering purity + REPL raw-term gating; A-S5.1–.7 the Lambda/Ambient PRODUCTION FLIP onto the generated in-Rho quiescence driver with its FV and docs closure; A-S6 (landed 2026-07-20) the DEMO FLIP + step-policy hygiene — every rho_net demo registers on the two-stage lazy machine wrapper, so the runtime mandate ("Dovetail handles only semantic predicates at runtime", plus labeled step introspection and lazy deferral reports) holds REGISTRY-WIDE (`repl/tests/a_s6_demo_registry_exec.rs`, `repl/tests/a_s6_demo_step_policy.rs`). Tracked as the pgmcp work item `track-a-runtime-boundary-enforcement-dovetail-semantic-predicates-only-4557ba5e` |
...
**per-iteration binder-float canonicalization (A-S5.8: the `^float` receiver
family — the S2 seed and every fired contractum float ON the reducer, with
capture avoidance by shift arithmetic rather than gensym)**, and the
whole-subject drive to rest are all COMMs on the live reducer, with a
...
predicates** (INV-14, by construction), and the **width-cast Tier-3 held-fold
trampoline** ([10 — Adaptive Evaluation Model](10-adaptive-evaluation-model.md)).
The A-S5.4a host boundary float ([26 §13](26-in-rho-ac-family-reference.md#13-the-ambient-fragment-cardelligordon-alignment))
is RETAINED but no longer load-bearing for redex exposure — it is
defense-in-depth plus the NewComm DISPLAY ordering (the α-canonical FIX-A
semantic-key minimization the erased reflected ABI retains as an α-quotient
function but for which no Match-expressible total order or hash over reflected
`Par`s exists — decision Q-NC: the float NF is unique up to the NewComm run
permutation, exposure is run-order-invariant by
`redex_invariant_under_run_permutation`, and the run-order-sensitive α exec
goldens ride exactly the retained host ordering, F8-AM-5b).
