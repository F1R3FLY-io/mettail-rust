...
| class explosion | The Datalog failure mode where the `proc` × `eq` × `rw` relations feed each other in a positive-feedback loop and the materialized fact count grows super-linearly (`25 → 625 → 100,000+` evaluations on the measured Rholang case). The e-graph collapses equal forms into one class and avoids it. |
