...
The adapter crate `dovetail-runtime` owns that projection so the
substrate-neutral `dovetail` crate does not depend on `runtime`. This
...
| Ascent | Former production rewrite backend; the generated Ascent engine was retired in P6. Only the fail-closed `Language::run_ascent` differential-oracle hook survives (`RuntimeBackend::Ascent`, never returned by `selected_default_runtime_backend`). |
| CESK runtime backend | Former runtime backend path, retired in P6 and replaced by Dovetail plus Rho-native execution. |
| `dovetail-runtime` | One-way adapter that installs Dovetail as a selected runtime backend and projects complete checked reports into `RuntimeBackendOutput::Dovetail`. |
