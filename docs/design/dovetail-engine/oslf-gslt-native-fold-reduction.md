# Dovetail as the Foundational Funding/GSLT Rewrite Engine — Native Fold Reduction
...
|---|---|---|
| Dovetail | The standalone MeTTaIL rewrite-engine crate (`dovetail/`); here cast as the foundational funding/GSLT engine. | [G-DT] |
| `language!` | The macro declaring a language's categories, syntax, equations, rewrites, **folds**, guards; expands to `LanguageDef`. | [G-DT] |
| **GSLT** | *Graph-structured lambda theory*: a language definition = syntax + equations + rewrites + operational laws. Dovetail's saturation **is** its reduction relation. | **[NEW]** §2.1 |
| **The funding discipline** | The cost/resource discipline governing which rewrites may fire; a cost-accounting extension of the rho calculus, distinct from OSLF (Operational Semantics in Logical Form). | **[NEW]** §2.2 |
...
| `is_funded(Δ, Σ, margin)` | The funding predicate `Δ + margin ≤ Σ`. | **[NEW]** §2.2 |
| the four funding laws | `sound`, `reject-underfunded`, `supply-monotone`, `decidable` — the obligations on `is_funded`, discharged in `MettaFundingLawsConformance.v`. | **[NEW]** §2.2 |
...
| **rigail** | Dovetail's weight-algebra module (semirings + Newton SCC solver) realizing funding cost. | **[NEW]** §2.2 |
...
**Thesis.** A fold is a **funded GSLT rewrite**. It must reduce *inside* Dovetail's
...
A **GSLT** (graph-structured lambda theory) is a language definition: a syntax, a set of
...
### 2.2 The funding discipline

**The funding discipline** is the cost discipline deciding *which*
...
(`MettaFundingLawsConformance.v`): `sound` (`funded ⇔ Σ ≥ Δ + margin`), `reject-underfunded`
...
Dovetail realizes the funding discipline directly:
...
> ![GSLT reduction ≅ funding-gated saturation](figures/12-gslt-oslf-correspondence.svg)
...
> ![Dovetail as the foundational funding/GSLT engine](figures/12-dovetail-oslf-gslt-engine.svg)
...
APPLY-NATIVE-FOLD(op = int-cast, egraph, σ):
  ...
  a ← reconstruct( best_derivation(a_class) )           # funded 1-best (funding selection)
...
> ![Reducing a fold inside funding-gated saturation](figures/12-native-fold-saturation-flow.svg)
...
selects the normal form — itself an act of funding (choosing the lower-cost derivation).
...
- **`DovetailSaturation.v`**: a native equality is a `good` generated fact, so
  ...
  `native_refire_is_noop` (a re-fired fold merges nothing). The **funding bridge**: a fold
...
4. **Funding** wired as the fold-transition weight + the budget-as-`Σ` bridge + the
   progress weight.
...
- **A — recursive `try_fold_to_self` method:** a reduction path *outside* Dovetail; not
  funded, not a GSLT rewrite in the engine; contradicts Dovetail's foundational role and
...
- These were the red-team's findings; **C (typed-`L`)** is the principled survivor — the one
  realization in which "Dovetail is the foundational funding/GSLT rewrite engine" is literally true.
