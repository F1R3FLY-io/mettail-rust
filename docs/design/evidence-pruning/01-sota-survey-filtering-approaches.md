...
Project artifacts cross-referenced: `formal/rocq/prattail_wpda_runtime/theories/{CastCompareFrontierBound.v, CastDelegateMergeBound.v, LexForkKeywordReservation.v, BareVarFanQuotient.v}`, `formal/rocq/dovetail/theories/{NBestExtraction.v, EnumerationCompleteness.v, InsideWeightSccClosure.v}`, `formal/rocq/egraph/theories/EGraphBudgetDedup.v`, `docs/design/parser-fv/{evidence-driven-early-pruning.md, evidence-gated-cross-cat-dispatch.md}`, `docs/design/evidence-pruning/00-existing-mechanisms-inventory.md`.
