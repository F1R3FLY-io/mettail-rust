...
| B: hand-assembled Pars — `connective_used=false` silently kills COMM; de Bruijn/`locally_free`/bind-sort conventions unspecified | H-B2 | production `spatial_matcher_pda.rs`; `match.rs:22-60`; `dispatch.rs:14-19`; `env.rs:33-47`; `reduce.rs:1090-1094`; `p_input_normalizer.rs:394-395` | **D1** (normalizer owns all of it); conventions recorded in §2 informational block for any future Par-direct work |
