...
- **PARALLEL-SESSION Rocq landing (assessed, kept, reclassified):** five zero-admission theories appeared in `formal/rocq/rho_bridge/theories/` during round 2 from a concurrent session (`CommReductionCorrespondence.v`, `RhoGroundingAndNames.v`, `RhoObservationFingerprint.v`, `AmbiguityWitnessEnumeration.v`, `RhoBackendFlipGate.v`) — all compile under the capped gate, none carries `Admitted`/`Axiom`/`Conjecture`. **Honest classification:** `CommReductionCorrespondence.v` models a MONOTONE fact-insertion semantics with persistent contracts (premises checked by membership, never consumed) — the M-RHO.2+ rules-as-data saturation layer, NOT .1's linear `Comm` (which consumes the matched sends + receive); it does not discharge §4 thms 2–6. `RhoObservationFingerprint.v` = a sound exact-key/order-irrelevance comparison kit (D3's discipline layer). `RhoGroundingAndNames.v` = a sound ν-freshness/no-capture kit (adjacent to, not equal to, `grounding_commutes`). `AmbiguityWitnessEnumeration.v` = the branch-per-alternative witness encoding (the M-RHO.3 mechanism). `RhoBackendFlipGate.v` = the M-RHO.4 flip-gate conjunction. All five are banked as kits with their stage assignments; **the .1-specific obligations (§4 thms 1–6 + fences, on a LINEAR consumption model) remain TO WRITE.** Claiming the landed files as the .1 obligations would be the exact over-claiming R6 fences.
...
| `PInputs` (:77) | `(n1?x1,…,nk?xk).{p}` | **COMM (receive half)** | renders the k-bind join `for(x1 <- ⟦n1⟧ & … & xk <- ⟦nk⟧){ ⟦p⟧ }` — atomic all-or-nothing rendezvous, the host's native polyadic join. The current .1 source-text path is green for distinct receive channels. Lower-level RSpace `consume` supports duplicate channel groups, but `evaluate_with_term` rejects duplicate receives with `Receiving on the same channels is currently not allowed`; same-channel duplicate joins therefore require either a parser-supported encoding or direct ADT lowering before they can enter the green source-text oracle. |
...
**Landed-kit map (§0-REVISION v3 — the five parallel-session theories vs these obligations):** `RhoObservationFingerprint.v` (exact-key/order-irrelevance) is the comparison-DISCIPLINE layer under the D3 gate — reusable as-is; `RhoGroundingAndNames.v` (ν-freshness/no-capture) supplies adjacent lemmas to thm 5 but is not `grounding_commutes`; `CommReductionCorrespondence.v` as landed models MONOTONE persistent-contract firing (premises by membership, no consumption) — the M-RHO.2+ rules-as-data layer; thms 2–3 here require a LINEAR model (COMM consumes the matched sends + receive — multiset rewriting, not fact insertion) and must be added (new section or sibling file) WITHOUT over-claiming the landed file; `AmbiguityWitnessEnumeration.v` = the M-RHO.3 branch-per-alternative encoding; `RhoBackendFlipGate.v` = the M-RHO.4 gate conjunction. None of the five discharges thms 1–6.
...
- **Recorded limit (multi-bind same-channel joins):** the .1 source-text host path rejects duplicate receive channels at normalization (`ReceiveOnSameChannelsError`, `p_input_normalizer.rs:408-423`) — these inputs never reach RSpace under D1, so they are not gated by the source-text oracle at all; they remain an explicit excluded case until M-RHO.3 supplies an ambiguity-preserving branch-per-alternative encoding or the bridge lowers directly to an accepted `Par`/receive representation. *(v2's claim that such a join's assignment is "hash-pinned — arrival order explores nothing" was REFUTED in round 2 — the produce path prepends the arriving datum after the hash sort (`rspace.rs:818-825`) and the hash itself ingests `thread_rng`-derived state in the harness context; see §0-REVISION v3 D7-CORRECTION. The harness must never rely on resting-candidate selection order.)*
...
2. **Differential oracle (.1.0).** `mettail-rho-runtime/tests/rho_comm_oracle.rs`: the §3 corpus (members 1–7, 9–16) green under the D3 membership gate, with member #8 (same-channel join) covered by an explicit parser-boundary regression asserting the `ReceiveOnSameChannelsError` class. Existing `rho_vs_ascent` + `run_calculator` stay green (no regression to M-RHO.0).
