...
| 2 | rhocalc `rhocalc_cast_int_invalid_width_error` | `{int(1,7)}` | stuck `int(1,7)` | contains `error` |
| 3 | rhocalc `rhocalc_cast_int_nonfinite_float_is_error` | `{int(0.0/0.0,8)}` | stuck | contains `error` |
| 4 | rhocalc `rhocalc_cast_fixed_floor` | `{fixed(3.49p2,1)}` | `17/5` | contains `3.4p1`/`3.4` |
| 5 | rhocalc `rhocalc_cast_float_from_rational_string` | `{float("1r/2r",32)}` | `1/2` | `0.5` |
...
non-finite (`cast_int_from_f64(NaN)`=`Err(NonFinite)`). The rhocalc wrapper
`rho_proc_int_bin` (numeric_dispatch.rs:423) ALREADY maps `None→Proc::Err`. The bug is
that the EVAL codegen DISCARDS the failure:

- **rhocalc** (`IntBinProc . a:Proc,w:Int : Proc`, non-native Proc result): the non-native
...
infallible) and rhocalc's 4 binary casts (IntBinProc/UIntBinProc/FloatBinProc/
...
(BigRat→Err; rhocalc Proc→Err), else (no error variant) keep the current fall-through.
...
**Fix 1b (rhocalc non-native path, mod.rs ~:3450-3460 binary arm):** when the rule is a
...
`.ok()?`), so `rhocalc_cast_float_overflow_to_inf` is unaffected.
...
based float-string tests (`rhocalc_cast_float_from_fixed_p_string` `"1000.1p1"`→asserts
...
- target 5 (rhocalc_tests.rs:945-949): assert NF `1/2` (== value of 0.5).
- target 4 (rhocalc_tests.rs:986-994): assert NF `17/5` (== value of 3.4).
...
3. Gates (each must hold):
   ...
   - the 5 targets PASS; the currently-passing cast/numeric tests stay green
     (calc test_cast_uint_*, test_cast_fixed_floor, test_bigint/bigrat_unary, rhocalc
     rhocalc_cast_int_float_floor, _uint_*, _float_overflow_to_inf, _float_from_*,
     _int_congruence_through_add, …).
4. Revert strategy: `git diff > /tmp/x.patch && git apply -R /tmp/x.patch` if any gate
   regresses uncleanly.

## Expected before→after
- targets 1,2,3: stuck → `cast_error_int` (calc) / `error` (rhocalc).
...
1. `macros/src/logic/mod.rs` (+98/−18, file was clean at HEAD):
   ...
   - **Fix 1b** (rhocalc non-native path, ~`:3400`): the per-rule `filter_err`
     ...
     `Err` variant is rhocalc `Proc`, so this is rhocalc-scoped.
2. `languages/tests/rhocalc_tests.rs`: targets 4,5 assertions corrected to the
...
- rhocalc op+unit+tests: failing-set diff vs M1/Step-A baseline = the 3 cast
  targets REMOVED, **zero new failures**. The remaining 19 rhocalc_tests fails are
...
Fix 1b also makes rhocalc div-by-zero / type-mismatch / fraction-zero GENUINELY
